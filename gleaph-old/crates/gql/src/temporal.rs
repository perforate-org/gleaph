...
/// Parse "YYYY-MM-DDTHH:MM:SS[.frac][Z|±HH:MM]" → (unix_seconds, subsec_nanos).
pub fn parse_datetime(s: &str) -> Option<(i64, u32)> {
    ...
    // Find where the time fraction ends and timezone begins
    let rest = &s[11..];
...
fn split_time_tz(s: &str) -> Option<(&str, &str)> {
    // Look for Z, +, or - after the seconds portion
    let b = s.as_bytes();
    if b.len() < 8 {
        return None;
    }
    // Minimum time is HH:MM:SS (8 chars), then optional .frac, then optional tz
    let mut i = 8;
...
pub fn parse_duration(s: &str) -> Option<(i32, i64)> {
    ...
    while i < b.len() {
        ...
        // Parse numeric value (possibly fractional)
        let start = i;
...
pub fn ymd_to_days(y: i32, m: u32, d: u32) -> Option<i32> {
    ...
    // Algorithm: count days from civil date to epoch.
    // Adapted from Howard Hinnant's algorithms.
    let y = y as i64;
    let m = m as i64;
    let d = d as i64;
...
mod tests {
    ...
    fn parse_datetime_offset() {
        ...
        // +09:00 means subtract 9 hours from local time to get UTC
