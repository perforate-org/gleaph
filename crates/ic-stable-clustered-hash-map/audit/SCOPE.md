...
when the insertion-point prefix is explicitly occupied; relocation-step preservation and
the Rust derivation of that prefix remain open.
...
part of the intermediate state, and full-chain composition remains open.
`InsertRelocateNoHolesOK` and `insertRelocate_preserves_noHoles` now compose those cases
across the full certified insert chain. The per-step prefix and distance premises remain
explicit and are not yet derived from Rust's scan loop.
