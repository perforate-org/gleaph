...
theorem badState_dist (i : Nat) (hi : i ≤ 4) : badState.dist i = i := by
  simp [badState, hi]

-- B1 holds: an occupied slot's distance does not exceed its position.
theorem badState_distanceValid : DistanceValid badState := by
  intro i hb hoc
  have hle4 : i ≤ 4 := by
    by_cases h : i ≤ 4
    · exact h
...
theorem badState_clusterOrdered : ClusterOrdered badState := by
  intro i j hbi hbj hij hoci hocj
  have hi : i ≤ 4 := by
    by_cases h : i ≤ 4
    · exact h
    · have hd : badState.dist i = EMPTY := by simp [badState, h]
...
theorem badState_notDistanceBounded : ¬ DistanceBounded badState := by
  intro db
  have h4 : 4 < capacity badState.n := by decide
  ...
  simp [badState] at h            -- h : 4 ≤ 3 → contradiction, closes goal
...
example : ¬ (∀ s : State, (DistanceValid s ∧ ClusterOrdered s) → DistanceBounded s) := by
  intro h
  have hv : DistanceValid badState ∧ ClusterOrdered badState :=
    ⟨badState_distanceValid, badState_clusterOrdered⟩
