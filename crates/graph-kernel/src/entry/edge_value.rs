...
pub enum EdgeValueEncoding {
    ...
    WeightBinary16,
    /// Opaque fixed-width payload; [`EdgeValueProfile::byte_width`] may be any positive width.
    RawBytes,
}
...
impl EdgeValueProfile {
    pub const fn no_value() -> Self {
        Self {
            byte_width: 0,
            encoding: EdgeValueEncoding::RawBytes,
        }
    }
    ...
    pub const fn opaque_bytes(byte_width: u16) -> Self {
        assert!(
            byte_width > 0,
            "opaque edge value byte width must be positive"
        );
        Self {
            byte_width,
            encoding: EdgeValueEncoding::RawBytes,
    ...
    pub fn validate(&self) -> Result<(), EdgeValueProfileError> {
        ...
        let ok = match &self.encoding {
            EdgeValueEncoding::RawBytes => true,
            _ if w == 0 => false,
            EdgeValueEncoding::RawU8 | EdgeValueEncoding::RawI8 => w == 1,
            ...
            | EdgeValueEncoding::WeightBinary16 => w == 2,
            ...
            EdgeValueEncoding::RawU128 | EdgeValueEncoding::RawI128 => w == 16,
            EdgeValueEncoding::RawFixed32 => w == 32,
            EdgeValueEncoding::RawFixed64 => w == 64,
    ...
    pub fn prepare(&self) -> Result<PreparedEdgeValueDecoder, EdgeValueProfileError> {
        ...
        Ok(match &self.encoding {
            ...
            EdgeValueEncoding::RawBytes => PreparedEdgeValueDecoder::RawBytes {
...
pub fn decode_edge_value(
    ...
    Ok(match decoder {
        ...
        PreparedEdgeValueDecoder::RawBytes { byte_width } => {
            let w = usize::from(*byte_width);
            if bytes.len() != w {
                return Err(EdgeValueProfileError::WidthEncodingMismatch);
            }
            DecodedEdgeValue::Bytes(bytes.to_vec())
        }
    })
}
...
mod tests {
    ...
    fn arbitrary_byte_width_validates_for_raw_bytes_profile() {
        let profile = EdgeValueProfile::opaque_bytes(12);
        profile.validate().expect("opaque width 12 valid");
        let dec = profile.prepare().expect("prepare");
        let payload: Vec<u8> = (0..12).map(|i| i as u8).collect();
        assert_eq!(
            decode_edge_value(&dec, &payload).expect("decode"),
            DecodedEdgeValue::Bytes(payload)
        );
    }

    #[test]
    fn no_value_profile_requires_raw_bytes_encoding() {
        assert!(EdgeValueProfile::no_value().validate().is_ok());
        let bad = EdgeValueProfile {
            byte_width: 0,
