...
/// Vector index group ordinal. Today a graph has a single vector target (group `None` = whole
/// graph); the future shard-group split (ADR 0031 readiness, vector-index.md "Multi-canister
/// readiness" A) assigns a `VectorIndexId` per contiguous graph shard group, mirroring
/// `IndexClusterId`. `0` is valid (the sole/whole-graph group).
#[derive(
    Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Hash, CandidType, Serialize, Deserialize,
)]
#[repr(transparent)]
#[serde(transparent)]
pub struct VectorIndexId(pub u32);
...
impl From<u32> for VectorIndexId {
    #[inline]
    fn from(value: u32) -> Self {
        Self(value)
    }
...
impl From<VectorIndexId> for u32 {
    #[inline]
    fn from(value: VectorIndexId) -> Self {
        value.0
    }
}

impl fmt::Display for VectorIndexId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.0.fmt(f)
    }
}

impl Storable for VectorIndexId {
    const BOUND: Bound = Bound::Bounded {
        max_size: 4,
        is_fixed_size: true,
    };
