//! Router data-plane wire contract shared by the client and canister SDKs.
//!
//! Owns the query/mutation response envelope, read freshness contracts, mutation tokens, the
//! Router error type, and the durable bulk-load command family. Mirror contracts in
//! `gleaph_graph_kernel` stay in lockstep with these Candid shapes; variant and field order are
...
/// Decimal row binding used by generated bindings.
///
...
/// Half-precision float row binding used by generated bindings.
///
/// Serde and Candid use the raw `u16` bit pattern ([`GqlWireValue::Float16`]).
#[cfg(feature = "f16")]
pub use gleaph_gql_ic_wire::GqlFloat16;
/// Signed 256-bit integer row binding used by generated bindings.
///
...
/// Unsigned 256-bit integer row binding used by generated bindings.
#[cfg(feature = "u256")]
pub use gleaph_gql_ic_wire::GqlUint256;
/// GQL `ZonedTime` row binding used by generated bindings.
///
/// Time of day with a fixed UTC offset; the binding keeps the `{nanos, offset_seconds}` wire
/// record form.
...
impl GqlQueryResult {
    ...
    ///
    /// This is the path used by generated bindings: each wire value is projected to JSON (see
    /// [`gql_wire_value_to_json`]) and deserialized with the row type's serde derives and renames.
...
// `gleaph-gql` default features) even when this crate's `nightly-f128` feature is disabled,
// while a standalone build has no Float128 variant.
#[allow(
    unreachable_patterns,
    reason = "Float128 depends on the nightly-f128 feature"
)]
...
pub enum RouterError {
    ...
    Busy {
        /// The named durable operation that must settle first.
        operation: String,
    },
    ...
    ExecutionPathMismatch {
        /// The canister entrypoint that was called.
        entrypoint: String,
        /// The GQL program kind that was parsed.
        program_kind: String,
        /// The actual call kind of the message.
        call_kind: String,
        /// Guidance for the caller to correct the mismatch.
        remedy: String,
    },
    ...
    GraphContextMismatch {
        /// The graph context requested by the API caller.
        api_graph: String,
        /// The graph resolved by the GQL program.
    ...
    ProjectionLag {
        /// Id of the lagging shard.
        shard_id: u32,
        /// The watermark that lags.
        watermark: String,
        /// Watermark level required to serve the read.
        required: u64,
        /// Watermark level currently reached.
        current: u64,
    },
    /// A federated update bundle contained more than one top-level DML statement.
    UnsupportedMultiDmlBundle {
        /// Number of top-level DML statements supplied.
        dml_statements: u32,
        /// Number of shards the bundle would touch.
        shard_count: u32,
    },
    ...
    AckConflict {
        /// The stored registry version that did not match.
        stored: u64,
    },
...
///
/// Mirrors `gleaph_bulk_load_api::AtomicInsertReceiptV1`.
#[derive(CandidType, Deserialize, Clone, Debug, PartialEq, Eq)]
pub struct AtomicInsertReceiptV1 {
    /// Number of logical insert operations committed.
    pub logical_operation_count: u64,
    /// Number of logical vertices committed.
    pub logical_vertex_count: u64,
    /// Number of logical edges committed.
...
pub struct AtomicInsertVertexV1 {
    /// Labels assigned to the inserted vertex.
    pub vertex_labels: Vec<String>,
    /// Initial property set for the inserted vertex.
...
pub enum BulkLoadCommand {
    ...
    Start {
        /// Logical graph name; `None` selects the caller's home graph.
        graph_name: Option<String>,
        /// Caller-scoped idempotency key for the job.
        client_bulk_key: String,
    },
    /// Append one chunk to an open bulk-load job.
    Append {
        /// Logical graph name; `None` selects the caller's home graph.
        graph_name: Option<String>,
        /// Caller-scoped idempotency key for the job.
    ...
    /// Finalize an open bulk-load job and publish its graph changes.
    Finalize {
        /// Logical graph name; `None` selects the caller's home graph.
        graph_name: Option<String>,
        /// Caller-scoped idempotency key for the job.
    ...
    /// Abort an open bulk-load job and discard its appended chunks.
    Abort {
        /// Logical graph name; `None` selects the caller's home graph.
        graph_name: Option<String>,
        /// Caller-scoped idempotency key for the job.
...
pub enum BulkLoadChunkV1 {
    /// A chunk containing only vertex inserts.
    Vertices(Vec<AtomicInsertVertexV1>),
    /// A chunk containing only edges referencing existing vertices.
...
pub struct BulkLoadEdgeV1 {
    ...
    /// Optional inline property bytes for the edge.
    pub inline_property: Option<Vec<u8>>,
    /// Initial edge property set.
...
pub struct BulkLoadPropertyEndpointV1 {
    /// Label of the referenced vertex.
    pub vertex_label: String,
    /// Property name used for the equality lookup.
    pub property_name: String,
...
pub enum BulkLoadResponse {
    /// The job was opened; the next chunk ordinal is the first to append.
    Started {
        /// Ordinal of the next chunk to append.
        next_chunk_index: u32,
    },
    /// One chunk was durably committed.
    Appended {
        /// Ordinal of the committed chunk.
        chunk_index: u32,
        /// Ordinal of the next chunk to append.
        next_offset: u32,
        /// Receipt summarizing the committed chunk.
        receipt: AtomicInsertReceiptV1,
    },
    /// The job was finalized and its graph changes published.
    FinalizeAccepted {
        /// Post-finalize job state.
        state: BulkLoadPublicStateV1,
    },
    /// The job was aborted and its chunks discarded.
    AbortAccepted {
        /// Post-abort job state.
        state: BulkLoadPublicStateV1,
    },
}
...
pub enum BulkLoadPublicStateV1 {
    /// The job is open and accepting chunks.
    Open,
    /// A chunk append is pending.
    AppendPending,
    /// Finalize is pending.
    FinalizePending,
    /// Abort is pending.
    AbortPending,
    /// The job completed and its changes are published.
    Completed,
    /// The job was aborted.
    Aborted,
...
pub struct BulkLoadStatusPage {
    ...
    /// Number of chunks whose downstream work completed.
    pub completed_chunk_count: u32,
    /// Wall-clock time at which the job reached a terminal state, if any.
    pub terminal_at_ns: Option<u64>,
    /// Wall-clock time at which the job's retained state expires, if any.
    pub expires_at_ns: Option<u64>,
    /// Committed chunk receipts in this page.
    pub receipts: Vec<BulkLoadChunkReceiptV1>,
    /// Cursor for the next status page, if any.
