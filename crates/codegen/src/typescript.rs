...
fn generate_typescript_ir(ir: &CodegenIr) -> Result<String, ManifestError> {
    ...
    out.push_str("import type { ApiValue");
    if has_updates {
        out.push_str(", GqlMutationResult");
    }
    if has_queries {
        out.push_str(", GqlQueryResult");
    }
    out.push_str(", GraphClient");
    if has_sorts {
