...
const ENCODE_ONE_LINE: &str = "let params = gleaph_cdk::encode_gql_params(params.into_gql_params()).map_err(|error| gleaph_cdk::CallError::Decode { message: format!(\"failed to encode GQL params: {error:?}\") })?;";

/// Render the `encode_gql_params` statement, wrapping the closure like rustfmt when the
/// one-line form exceeds the default width.
fn encode_statement() -> &'static str {
    if super::format::exceeds_width(ENCODE_ONE_LINE, super::format::MAX_WIDTH - 8) {
