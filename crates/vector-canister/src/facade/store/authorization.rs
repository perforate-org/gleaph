...
use super::mutation::tombstone_slot;
...
fn assert_attach_allowed(shard_id: ShardId) -> Result<(), VectorCanisterError> {
...
fn begin_or_resume_detach(shard_id: ShardId) -> Result<u64, VectorCanisterError> {
...
fn finish_detach(shard_id: ShardId, generation: u64) -> Result<(), VectorCanisterError> {
    OWNERSHIP_CONFIG.with_borrow_mut(|cell| {
        let mut config = cell.get().clone();
        let active = config
            .active_detaches
...
pub(crate) fn init_from_args(args: &VectorCanisterInitArgs) -> Result<(), VectorCanisterError> {
    ...
    subject_store::create_for_install(args.subject_map_seed).map_err(VectorCanisterError::from)?;
...
pub(crate) fn reset_for_test_or_bench(
    ...
    definition_store::bind_for_fixture(args.definition_map_seed)
        .map_err(VectorCanisterError::from)?;
    subject_store::bind_for_fixture(args.subject_map_seed).map_err(VectorCanisterError::from)?;
    ...
    reset_definition_dependent_state(expected_incarnation, subject_incarnation)
...
#[cfg(any(test, feature = "canbench"))]
pub(crate) fn reset_definition_dependent_state(
...
pub(crate) fn open_definition_store_after_upgrade() {
...
pub(crate) fn open_subject_store_after_upgrade() {
...
pub(super) fn commit_attach_shard_canister(
    ...
    assert_attach_allowed(shard_id)?;
...
fn commit_detach_shard_step_with_budget(
    ...
    let (cursor, generation) = match resume {
        Some(cursor) => {
            let generation = validate_detach_resume(shard_id, cursor.detach_generation)?;
        ...
        None => {
            let generation = begin_or_resume_detach(shard_id)?;
    ...
    let step = purge_subjects_step(shard_id, &cursor.resume_key, budget)?;
    ...
    if next.is_none() {
        finish_detach(shard_id, generation)
...
pub(crate) fn admin_attach_shard_canister(
    ...
    commit_attach_shard_canister(graph_id, shard_id, shard_canister_principal)
}
...
pub(crate) fn admin_detach_shard_canister(
    ...
    assert_router_caller(caller)?;
    commit_detach_shard_step_with_budget(shard_id, resume, MAX_DETACH_EXAMINE_PER_STEP)
}
...
pub(crate) fn detach_shard_step_for_test(
    ...
    commit_detach_shard_step_with_budget(shard_id, resume, budget)
}

pub(super) fn assert_router_caller(caller: Principal) -> Result<(), VectorCanisterError> {
...
pub(crate) fn attach_single_shard_for_test(
    ...
    admin_attach_shard_canister(router, GraphId::from_raw(1), shard_id, shard_canister)
