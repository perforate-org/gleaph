...
pub(crate) mod graph_type_catalog_vocabulary {
    ...
    fn unregister_rejects_exact_pending_vector_work_without_lifecycle_mutation() {
        ...
        let intent = vector_ingest_outbox::intent_for_test(
            vector_ingest_outbox::NewVectorIngestIntent {
                graph_id,
                graph_target: before.graph_canister,
        ...
        vector_ingest_outbox::insert_intents_for_test(&[intent])
    ...
    fn vector_outbox_append_revalidates_live_target_and_definition_atomically() {
        ...
        use gleaph_graph_kernel::vector_index::{
            IndexedEmbeddingSpec, VectorEncoding, VectorMetric,
        };
        ...
        let first = vector_ingest_outbox::admit_awaiting_graph(vec![input()])
            .expect("valid live target and definition are accepted")
            .pop()
            .expect("one admitted intent");
        let before_rejection = vector_ingest_outbox::scan(None, 16).0;
        let counter_before_rejection =
        ...
        let err = vector_ingest_outbox::admit_awaiting_graph(vec![input()])
            .expect_err("a stale non-live shard must be rejected before append");
        assert!(err.contains("not live at exact Graph target"), "{err}");
        assert_eq!(
        ...
        assert!(err.contains("not attached to exact Vector target"), "{err}");
        assert_eq!(
            vector_ingest_outbox::scan(None, 16).0,
            before_rejection,
        ...
        assert_eq!(
            crate::facade::stable::ROUTER_MUTATION_COUNTER.with_borrow(|counter| *counter.get()),
            counter_before_rejection,
            "every rejected admission must preserve the mutation counter"
        );
        let second = vector_ingest_outbox::admit_awaiting_graph(vec![input()])
...
mod provisioning_tests {
    ...
    fn owner(request_id: &str, deployment_id: &str, _fingerprint: &str) -> IntentLockOwner {
        IntentLockOwner::new(ProvisioningRequestKey::new(
    ...
    fn intent_lock_release() {
        ...
        s.insert("deploy-a", req).expect("insert");
        assert!(s.intent_locked(&key, &owner("req-1", "deploy-a", "fp-1")));
        s.clear_request(&ProvisioningRequestKey::new(
            &test_request_id("req-1"),
            "deploy-a",
        ))
        .expect("clear");
        assert!(!s.intent_locked(&key, &owner("req-1", "deploy-a", "fp-1")));
    ...
    fn cross_request_intent_exclusion_does_not_mutate_b() {
        ...
        assert!(
            s.get_by_request_id(&ProvisioningRequestKey::new(
        ...
        assert_eq!(listed.len(), 1);
        assert_eq!(listed[0].request_id, test_request_id("req-a"));
    ...
    fn clear_request_absent_returns_not_found() {
        ...
        let absent_key = ProvisioningRequestKey {
            request_id: test_request_id("does-not-exist"),
    ...
    fn list_by_graph_handles_max_unicode_request_id() {
        ...
        assert!(
            listed
                .iter()
                .any(|r| r.request_id == test_request_id("\u{10ffff}req"))
        );
        assert!(
            listed
                .iter()
                .any(|r| r.request_id == test_request_id("normal-req"))
        );
    }
    ...
    fn commit_ack_preflight_rejects_lock_with_wrong_owner() {
        ...
        let intent_key =
            ProvisioningIntentKey::new(deployment_id, LogicalResource::GraphShard(ShardId::new(0)));
        let impostor = IntentLockOwner::new(ProvisioningRequestKey::new(
            &test_request_id("other-request"),
            deployment_id,
        ));
        ROUTER_PROVISIONING_INTENT_LOCK.with_borrow_mut(|locks| {
    ...
    fn release_intent_locks_owned_by_does_not_remove_foreign_lock() {
        ...
        let foreign = IntentLockOwner::new(ProvisioningRequestKey::new(
            &test_request_id("foreign-request"),
            deployment_id,
        ));
        ROUTER_PROVISIONING_INTENT_LOCK.with_borrow_mut(|locks| {
        ...
        let local_record = s
            .get_by_request_id(&ProvisioningRequestKey::new(
                &test_request_id(request_id),
                deployment_id,
            ))
            .expect("local record");
