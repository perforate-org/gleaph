...
impl RouterIndexClient {
    ...
    /// One batched equality lookup round trip. The index answers a prefix of `specs` at bucket
    /// granularity under the shared response payload budget and returns the resume cursor; the
    /// caller (see [`crate::index_lookup`]) follows the cursor and per-bucket paging.
    ...
    pub async fn lookup_range_page(
        &self,
        req: gleaph_graph_kernel::index::LookupRangePageRequest,
    ) -> Result<PostingHitPage, String> {
        #[cfg(target_family = "wasm")]
        {
            use ic_cdk::call::Call;
