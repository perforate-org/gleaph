...
use crate::facade::stable::ROUTER_LABEL_BACKFILL_STATE;
use crate::facade::stable::label_backfill::LabelBackfillShardState;
...
fn load_shard_state(shard_id: u32) -> LabelBackfillShardState {
...
fn store_shard_state(shard_id: u32, cursor: LabelBackfillShardState) {
    ROUTER_LABEL_BACKFILL_STATE.with_borrow_mut(|map| {
        map.insert(shard_id, cursor);
    });
}

pub(crate) fn build_backfill_args(
...
pub(crate) fn advance_backfill_cursor(
...
fn resolve_shard_for_graph(
    store: &RouterStore,
    logical_graph_name: &str,
    shard_id: u32,
) -> Result<ShardRegistryEntry, RouterError> {
    let entry = store.resolve_shard(shard_id)?;
    ...
    Fut: Future<Output = Result<LabelPostingBackfillResult, String>>,
{
    ...
    let shard = resolve_shard_for_graph(store, &args.logical_graph_name, args.shard_id)?;
    let mut cursor = load_shard_state(args.shard_id);
    if cursor.done {
    ...
    store_shard_state(args.shard_id, cursor);
...
pub(crate) fn admin_list_label_backfill_status(
    ...
    let mut out: Vec<LabelBackfillShardStatus> = shards
        .into_iter()
        .map(|shard| {
            let cursor = load_shard_state(shard.shard_id);
...
mod tests {
    ...
    fn admin_label_backfill_step_advances_cursor() {
        ...
        let result = futures::executor::block_on(admin_label_backfill_step(
            &store,
            admin,
        ...
        let status =
            admin_list_label_backfill_status(&store, admin, "tenant.main").expect("status");
    ...
    fn admin_label_backfill_step_rejects_wrong_graph() {
        ...
        let err = futures::executor::block_on(admin_label_backfill_step(
            &store,
            admin,
            AdminLabelBackfillStepArgs {
