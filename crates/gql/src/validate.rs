    ...
    for part in &lq.parts {
        ...
        match part {
            ...
            SimpleQueryStatement::Filter(f) => {
                validate_expr(&f.condition, &scope, &graph_scope)?;
            }
            SimpleQueryStatement::Let(l) => {
                validate_let(&l.bindings, &mut scope, &graph_scope)?;
            }
            ...
            SimpleQueryStatement::InlineProcedureCall(_ipc) => {
                validate_inline_scope_vars(_ipc)?;
                validate_inline_procedure_call(_ipc, &scope, &graph_scope)?;
                let outer_scope = scope.clone();
                collect_inline_procedure_result_bindings(_ipc, &outer_scope, &mut scope)?;
            }
            SimpleQueryStatement::Focused { graph, body } => {
                validate_graph_reference(graph, &scope, &graph_scope)?;
                if let Some(inner) = body {
                    // Validate the body in the same way as a top-level part.
                    // Recursive validation would require a helper; for now we
                    // validate the most common case (Match inside Focused).
                    match inner.as_ref() {
                        SimpleQueryStatement::Match(m) => {
            ...
            SimpleQueryStatement::Delete(del) => {
                validate_delete_vars(del, &scope, &graph_scope)?;
            }
        }
    }
...
fn validate_return(
    ...
    match &ret.body {
        ...
        ReturnBody::Items {
            ...
            // Build an extended scope that includes RETURN aliases for ORDER BY.
    ...
    for part in &query.parts {
        match part {
            ...
            SimpleQueryStatement::Focused { body, .. } => {
                // Collect bindings from the focused body if present.
                if let Some(inner) = body {
                    match inner.as_ref() {
                        SimpleQueryStatement::Match(m) => {
                            let mut match_scope = scope.clone();
...
fn expr_is_group_compatible(expr: &Expr, group_items: &[Expr]) -> bool {
...
fn collect_pattern_bindings(pattern: &GraphPattern, scope: &mut RapidHashSet<String>) -> VResult {
...
fn collect_path_primary_bindings(
    ...
    match primary {
        ...
        PathPrimary::Parenthesized { expr, .. } => {
            collect_path_expr_bindings(expr, scope)?;
        }
        PathPrimary::Simplified(_) => {
            // Simplified patterns don't introduce named bindings.
        }
    }
    Ok(())
}
