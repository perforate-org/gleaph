...
fn test_search_where_mixed_equality_and_range_accepted_by_planner() {
    ...
    assert!(
        filter.is_some(),
        "accepted mixed equality/range filter on distinct properties must be preserved in the plan"
    );
}

#[test]
fn test_search_where_non_leading_mixed_equality_and_range_accepted_by_planner() {
    let plan = plan_query(
        "MATCH (a:Author)-[:WROTE]->(d:Document) \
         SEARCH d IN ( \
           VECTOR INDEX document_embedding \
           FOR $query \
           WHERE d.category = $category AND d.price >= $min_price \
           LIMIT 100 \
         ) SCORE AS similarity \
...
fn test_search_where_mixed_equality_and_range_rejects_same_property() {
    let err = plan_query_err(
        "MATCH (d:Document) \
         SEARCH d IN ( \
           VECTOR INDEX document_embedding \
           FOR $query \
           WHERE d.price = 50 AND d.price >= 10 \
           LIMIT 100 \
         ) SCORE AS similarity \
         RETURN d, similarity",
    );
    assert!(
        ...
        "same-property mixed equality/range must be rejected, got {err}"
    );
}
    ...
    let plan = plan_query(
        "MATCH (d:Document) \
         SEARCH d IN ( \
           VECTOR INDEX document_embedding \
           FOR $query \
           WHERE d.category = 1 AND d.price >= 10 AND d.price < 100 \
           LIMIT 100 \
         ) SCORE AS similarity \
         RETURN d, similarity",
    );
...
fn test_search_where_two_sided_range_rejects_equality_plus_range() {
    ...
    assert!(
        err.to_string().contains("distinct properties"),
        "same-property equality-plus-range must be rejected, got {err}"
    );
}
...
#[test]
fn test_gleaph_weight_equality_fuses_to_edge_inline_property_predicate() {
    let plan =
    ...
    let value_eq = plan.ops.iter().find_map(|op| match op {
        ...
        | PlanOp::ExpandFilter {
            edge_inline_property_predicate,
            ..
        } => edge_inline_property_predicate.as_ref(),
        _ => None,
    });
    let Some(pred) = value_eq else {
        panic!(
            "expected edge inline value predicate on Expand, got ops={:?}",
            plan.ops
        );
    };
...
#[test]
fn test_gleaph_weight_gt_fuses_to_edge_inline_property_predicate() {
    let plan =
        plan_query("MATCH (a:Person)-[e:REL]->(b:Person) WHERE GLEAPH.WEIGHT(e) > 7 RETURN a, b");
...
#[test]
fn test_gleaph_vector_l2_fuses_to_edge_inline_vector_predicate() {
    let plan = plan_query(
    ...
    let Some(pred) = value_pred else {
        panic!(
            "expected edge inline vector predicate on Expand, got ops={:?}",
            plan.ops
        );
    };
...
#[test]
fn test_gleaph_vector_dot_fuses_to_edge_inline_vector_predicate() {
    let plan = plan_query(
        "MATCH (a:Person)-[e:REL]->(b:Person) \
...
#[test]
fn test_gleaph_vector_flipped_l2_fuses_to_edge_inline_vector_predicate() {
    let plan = plan_query(
        "MATCH (a:Person)-[e:REL]->(b:Person) \
