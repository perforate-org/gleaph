    ...
    ML: Memory,
{
    ...
    pub(super) edges: EdgeStore<E, MC, ME, ML>,
    ...
    ML: Memory,
{
    ...
    pub fn init(vertices: MV, counts: MC, edges: ME, log: ML) -> Result<Self, InitError> {
        Ok(Self {
            vertices: VertexStore::init(vertices).map_err(InitError::Vertices)?,
            edges: EdgeStore::init(counts, edges, log).map_err(InitError::Edges)?,
            _marker: PhantomData,
        })
    }
    ...
    pub fn edges(&self) -> &EdgeStore<E, MC, ME, ML> {
        &self.edges
    }

    pub fn into_memories(self) -> (MV, MC, ME, ML) {
        let (counts, edges, log) = self.edges.into_memories();
        (self.vertices.into_memory(), counts, edges, log)
    }
    ...
    fn rebalance_after_insert(&self, src: VertexId) -> Result<(), &'static str> {
        ...
        {
            self.resize().map_err(|_| "resize failed")?;
        }
    ...
    fn calculate_positions(&self, start_vertex: u64, end_vertex: u64, gaps: u64) -> Vec<u64> {
        ...
        let mut index = self.vertices.get(start_vertex).base_slot_start() as f64;
        for vid in start_vertex..end_vertex {
