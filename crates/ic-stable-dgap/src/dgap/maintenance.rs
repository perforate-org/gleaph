...
#[derive(Debug)]
pub struct DeferredDgap<E, V, MV, MC, ME, ML, MMQ, MDS>
where
    E: CsrEdge,
    V: CsrVertex,
    MV: Memory,
    ...
    MDS: Memory,
{
    graph: Dgap<E, V, MV, MC, ME, ML>,
    maintenance: MaintenanceQueue<MMQ, MDS>,
    config: DeferredConfig,
}

impl<E, V, MV, MC, ME, ML, MMQ, MDS> DeferredDgap<E, V, MV, MC, ME, ML, MMQ, MDS>
where
    E: CsrEdge,
    V: CsrVertex,
    MV: Memory,
    ...
    MDS: Memory,
{
    ...
    pub fn init_with_config(
        ...
        Ok(Self {
            graph: Dgap::init(vertices, counts, edges, log).map_err(DeferredInitError::Graph)?,
    ...
    pub fn graph(&self) -> &Dgap<E, V, MV, MC, ME, ML> {
        &self.graph
    }
    ...
    pub fn into_memories(self) -> (MV, MC, ME, ML, MMQ, MDS) {
        let (vertices, counts, edges, log) = self.graph.into_memories();
        let (queue, dirty) = self.maintenance.into_memories();
        (vertices, counts, edges, log, queue, dirty)
    }
