...
//!
//! - [`dgap`] — byte layouts for the **two** DGAP `M_e` memories (PMA `SEC` + `VCE` header + slab + logs).
...
pub use dgap::{
    ...
    EDGE_REGION_VERSION, PMA_SEGMENT_EDGE_COUNTS_MAGIC, SUGGESTED_ELEM_CAPACITY_MULTIPLIER,
    SUGGESTED_MIN_ELEM_CAPACITY, required_edges_and_log_bytes, required_segment_edge_counts_bytes,
    suggested_format,
};
pub use log_region::{
