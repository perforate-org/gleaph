...
//!
//! The [`prepass`] / [`fold_unit`] / [`fold_units`] functions are the canonical
//! Japanese text normalization used by the text-canister analyzers (plan 0339,
//! ported here in plan 0340): true NFKC + lowercase + variation-selector strip
//! (pre-tokenization) and the kana counter-variant fold ヶ/ヵ → ケ (emitted units
//! only). The [`Normalizer`] struct below is a separate, older hand-rolled
//! "simplified NFKC" implementation that is currently dead code (nothing calls it)
//! and is out of scope for this module's canonical path.
...
// -- Canonical Japanese text normalization (plan 0339, ported here in plan 0340) -------------
...
pub fn fold_unit(unit: &mut String) {
...
pub fn fold_units(units: &mut [String]) {
    for unit in units {
        fold_unit(unit);
    }
}
...
/// (the text-canister mecab dictionary field-scan pins the same predicate the
/// strip applies).
pub fn is_variation_selector_for_scan(c: char) -> bool {
...
mod tests {
    ...
    // -- Canonical prepass/fold (plan 0339, ported here in plan 0340) ------------------------

    /// IVS-free text is byte-identical to the former per-copy behavior
    /// (NFKC + lowercase, nothing else).
    #[test]
