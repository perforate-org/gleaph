...
impl PreparedEdgeVectorKernel {
    ...
    pub(crate) fn score(
        &self,
        edge_inline_property_bytes: &[u8],
        query: &[f32],
        metric: EdgeVectorMetric,
    ) -> Option<f32> {
        if query.len() != self.dims || edge_inline_property_bytes.len() < self.byte_width() {
            return None;
        }
        let bytes = &edge_inline_property_bytes[..self.byte_width()];
        match metric {
