...
use gleaph_graph_kernel::entry::{
    EdgeValueEncoding, EdgeValueProfile, PreparedEdgeValueDecoder, decode_edge_weight,
};
use half::f16;
...
impl PreparedEdgeValueBatchKernel {
    pub(crate) fn new(byte_width: u16, encoding: EdgeValueEncoding) -> Self {
        Self {
            byte_width,
            encoding,
        }
    ...
    pub(crate) fn collect_equal_value_indices(
        ...
        match width {
            1 => collect_equal_w1(value_bytes, needle[0], out),
            2 => collect_equal_w2(value_bytes, needle, out),
            4 => collect_equal_w4(value_bytes, needle, out),
            8 => collect_equal_w8(value_bytes, needle, out),
            16 => collect_equal_w16(value_bytes, needle, out),
            _ => collect_equal_fixed_width(value_bytes, needle, width, out),
        }
    }
    ...
    pub(crate) fn collect_matching_value_indices(
        ...
        if matches!(self.encoding, EdgeValueEncoding::F16) && width == 2 {
            collect_cmp_f16(value_bytes, op, needle, out);
            return;
        }
        if self.is_unsigned_integer_encoding() {
            match width {
                1 => collect_cmp_u8(value_bytes, op, needle[0], out),
                2 => collect_cmp_u16(value_bytes, op, needle, out),
                4 => collect_cmp_u32(value_bytes, op, needle, out),
                8 => collect_cmp_unsigned_scalar(value_bytes, op, needle, 8, out),
                16 => collect_cmp_u128(value_bytes, op, needle, out),
        ...
        if self.is_signed_integer_encoding() {
            match width {
                1 => collect_cmp_i8(value_bytes, op, needle[0] as i8, out),
                2 => collect_cmp_i16(value_bytes, op, needle, out),
                4 => collect_cmp_i32(value_bytes, op, needle, out),
                8 => collect_cmp_i64(value_bytes, op, needle, out),
                16 => collect_cmp_i128(value_bytes, op, needle, out),
                _ => collect_cmp_fixed_width_bytes(value_bytes, op, needle, width, out),
            }
            return;
        }
        if matches!(self.encoding, EdgeValueEncoding::F32) && width == 4 {
            collect_cmp_f32(value_bytes, op, needle, out);
            return;
        }
        if matches!(self.encoding, EdgeValueEncoding::F64) && width == 8 {
            collect_cmp_f64(value_bytes, op, needle, out);
            return;
        }
    ...
    fn is_signed_integer_encoding(&self) -> bool {
        matches!(
            self.encoding,
            EdgeValueEncoding::RawI8
                | EdgeValueEncoding::RawI16
                | EdgeValueEncoding::RawI32
                | EdgeValueEncoding::RawI64
                | EdgeValueEncoding::RawI128
        )
    }
...
fn collect_equal_fixed_width(
    ...
    for (idx, chunk) in value_bytes.chunks_exact(width).enumerate() {
        if chunk == needle {
            out.push(idx);
        }
    }
...
fn collect_equal_w1(value_bytes: &[u8], needle: u8, out: &mut Vec<usize>) {
    for (idx, &byte) in value_bytes.iter().enumerate() {
        if byte == needle {
            out.push(idx);
        }
    }
...
fn collect_equal_w2(value_bytes: &[u8], needle: &[u8], out: &mut Vec<usize>) {
    ...
    for (idx, chunk) in value_bytes.chunks_exact(2).enumerate() {
...
fn collect_equal_w8(value_bytes: &[u8], needle: &[u8], out: &mut Vec<usize>) {
    ...
    for (idx, chunk) in value_bytes.chunks_exact(8).enumerate() {
...
fn collect_equal_w16(value_bytes: &[u8], needle: &[u8], out: &mut Vec<usize>) {
    ...
    for (idx, chunk) in value_bytes.chunks_exact(16).enumerate() {
...
fn collect_cmp_i8(value_bytes: &[u8], op: CmpOp, needle: i8, out: &mut Vec<usize>) {
    for (idx, &byte) in value_bytes.iter().enumerate() {
...
fn collect_cmp_i16(value_bytes: &[u8], op: CmpOp, needle: &[u8], out: &mut Vec<usize>) {
...
fn collect_cmp_i32(value_bytes: &[u8], op: CmpOp, needle: &[u8], out: &mut Vec<usize>) {
...
fn collect_cmp_i64(value_bytes: &[u8], op: CmpOp, needle: &[u8], out: &mut Vec<usize>) {
...
fn collect_cmp_i128(value_bytes: &[u8], op: CmpOp, needle: &[u8], out: &mut Vec<usize>) {
...
fn collect_cmp_u128(value_bytes: &[u8], op: CmpOp, needle: &[u8], out: &mut Vec<usize>) {
...
fn collect_cmp_f16(value_bytes: &[u8], op: CmpOp, needle: &[u8], out: &mut Vec<usize>) {
...
fn collect_cmp_weight(
    ...
    for (idx, chunk) in value_bytes.chunks_exact(usize::from(width)).enumerate() {
...
fn weight_decoder(
    width: u16,
    encoding: &EdgeValueEncoding,
) -> Result<PreparedEdgeValueDecoder, gleaph_graph_kernel::entry::EdgeValueProfileError> {
    EdgeValueProfile {
        byte_width: width,
...
fn collect_cmp_f32(value_bytes: &[u8], op: CmpOp, needle: &[u8], out: &mut Vec<usize>) {
...
fn collect_cmp_f64(value_bytes: &[u8], op: CmpOp, needle: &[u8], out: &mut Vec<usize>) {
...
mod tests {
    ...
    fn equal_w1_collects_indices() {
        let kernel = PreparedEdgeValueBatchKernel::new(1, EdgeValueEncoding::RawU8);
    ...
    fn equal_w2_collects_indices() {
        let kernel = PreparedEdgeValueBatchKernel::new(2, EdgeValueEncoding::RawU16);
    ...
    fn equal_w4_collects_indices() {
        let kernel = PreparedEdgeValueBatchKernel::new(4, EdgeValueEncoding::RawU32);
    ...
    fn equal_w8_collects_indices() {
        let kernel = PreparedEdgeValueBatchKernel::new(8, EdgeValueEncoding::RawU64);
    ...
    fn equal_w16_collects_indices() {
        let kernel = PreparedEdgeValueBatchKernel::new(16, EdgeValueEncoding::RawU128);
    ...
    fn equal_arbitrary_width_uses_memcmp() {
        let kernel = PreparedEdgeValueBatchKernel::new(12, EdgeValueEncoding::RawBytes);
        let needle = [7u8; 12];
    ...
    fn raw_i128_ordering_is_numeric() {
        let kernel = PreparedEdgeValueBatchKernel::new(16, EdgeValueEncoding::RawI128);
        let mut buf = Vec::new();
    ...
    fn f16_ordering_is_decoded_float_ordering() {
        let kernel = PreparedEdgeValueBatchKernel::new(2, EdgeValueEncoding::F16);
    ...
    fn weight_ordering_uses_decoded_weight_semantics() {
        let kernel = PreparedEdgeValueBatchKernel::new(
            2,
            EdgeValueEncoding::WeightLinearU16 { min: 0.0, max: 1.0 },
        );
        let mut buf = Vec::new();
