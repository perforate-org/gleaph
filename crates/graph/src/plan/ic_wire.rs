...
pub use gleaph_gql_ic::{GqlWireDecodeError, GqlWireRow, GqlWireRows};

use super::query::PlanQueryResult;

pub fn ic_wire_from_plan_query_result(
    result: &PlanQueryResult,
) -> Result<GqlWireRows, GqlWireDecodeError> {
    GqlWireRows::try_from_value_rows(&result.rows)
}

pub fn plan_query_result_from_ic_wire(
    wire: GqlWireRows,
) -> Result<PlanQueryResult, GqlWireDecodeError> {
    Ok(PlanQueryResult {
...
mod tests {
    ...
    fn plan_query_wire_round_trips_candid_blob() {
        ...
        let decoded = GqlWireRows::decode_blob(&bytes).expect("decode");
