...
use crate::facade::bench_stable_layout::{
    edge_profile_label, install_edge_profile_fixtures, read_inline_property_profile_via_store,
};
...
#[bench(raw)]
fn bench_layout_edge_inline_property_profile_read() -> canbench_rs::BenchResult {
    install_edge_profile_fixtures();
    let label = edge_profile_label();
    canbench_rs::bench_fn(|| {
        let _scope = canbench_rs::bench_scope("layout_edge_profile_read");
        read_inline_property_profile_via_store(label);
    })
}
