...
fn resolve_scan_value_to_value(
    ...
    match value {
        ...
        ScanValue::TextPrefix(_) => Err(GqlRunError::Plan(
            "complete-prefix seed validation does not support text-prefix scan values".to_owned(),
        )),
    }
}
...
mod tests {
    ...
    fn adhoc_gleaph_cost_selects_weighted_shortest_path() {
        ...
        let out = pollster::block_on(run_adhoc_gql(
            store,
            "MATCH p = ANY SHORTEST (a:WgtGqlA)-[e:WgtGqlRoad]->{1,5}(c:WgtGqlC) COST BY e.distance RETURN p",
            &params,
            None,
            GqlCanisterExecutionMode::CompositeQuery,
            GqlExecutionContext::default(),
    ...
    fn adhoc_gleaph_cost_shortest_k_returns_weighted_paths() {
        ...
        let out = pollster::block_on(run_adhoc_gql(
            store,
            "MATCH SHORTEST 2 (a:WgtGqlA)-[e:WgtGqlRoad]->{1,5}(c:WgtGqlC) COST BY e.distance RETURN c",
            &params,
            None,
            GqlCanisterExecutionMode::CompositeQuery,
            GqlExecutionContext::default(),
