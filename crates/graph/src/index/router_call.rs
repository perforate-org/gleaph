//! Async graph → router inter-canister calls on Wasm (`ic-cdk` `Call` API).
...
pub(crate) async fn call_router0<R: candid::CandidType + for<'de> candid::Deserialize<'de>>(
    router: Principal,
    method: &'static str,
) -> Result<R, String> {
...
pub(crate) async fn call_router1<
    T: candid::CandidType,
...
pub(crate) async fn call_router2<
