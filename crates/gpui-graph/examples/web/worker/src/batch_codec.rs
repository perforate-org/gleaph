...
//! the web example's own choice for `GraphBatch<String, String, String,
//! String>`, plus the envelope tags that distinguish application messages from
...
//!   target key), one direction byte, and a length-prefixed label string.
...
mod tests {
    ...
    fn corrupted_batch_forms_fail_closed_without_partial_output() {
        ...
        assert!(
            matches!(decode_merge_batch(&bytes), Err(CodecError::BadDirection(9))),
    ...
    #[test]
    fn envelope_tags_stay_distinct_from_library_requests() {
