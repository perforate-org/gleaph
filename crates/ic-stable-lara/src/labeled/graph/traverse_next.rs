    ...
    M: Memory,
{
        ...
        E: CsrEdgeTombstone,
    {
        ...
        // Fast path for hybrid buckets: prefetch the overflow-log replay, verify the
        // bucket has no tombstones in the overflow log, then bulk-read the edge slab and
        // the inline-property bytes span and walk the three logical regions (overflow log,
        // edge-slab entries with log-backed values, dense value-slab prefix) directly.
        if bucket.overflow_log_head() >= 0 {
            let reserved = self.bucket_reserved_edge_slots(owner, &bucket);
            if reserved == bucket.degree() {
                return self.visit_hybrid_label_bucket_edges_with_inline_property(
                    owner, label, &bucket, order, reserved, width, visit,
                );
            }
        }

        // Generic fallback: use the streaming labeled iterator which reads per-slot
        // inline property bytes into the edge value, then copy them back into
        // `InlinePropertyBytes`.  Used for hybrid buckets with overflow-log tombstones.
        let mut iter =
    ...
    /// Visits every live edge together with its inline-property bytes in a hybrid
    /// label bucket by combining an overflow-log replay with bulk reads of the edge slab
    /// and the inline-property-bytes span.
    ///
    /// `reserved` must equal `bucket.degree()`; this path does not handle overflow-log
    /// tombstones and falls back to the streaming iterator when they are present.
    #[inline]
    fn visit_hybrid_label_bucket_edges_with_inline_property<B>(
        &self,
        owner: VertexId,
        label: BucketLabelKey,
        bucket: &LabelBucket,
        ...
        E: CsrEdgeTombstone,
    {
        ...
        let value_slab_slots = bucket.inline_property_bytes_slab_slots().min(slab_slots);
        let log_chains = self.bucket_inline_property_bytes_log_chain_opt(owner, bucket);

        // Bulk-read the edge slab so we can decode edge rows without per-slot round trips.
        let stored_slots = bucket.stored_slots;
        let edge_bytes_len =
            (stored_slots as usize)
                .checked_mul(E::BYTES)
        ...
        // Bulk-read the dense value-slab prefix that backs the lowest live ordinals.
        let value_prefix_bytes = if width > 0 {
            self.read_bucket_inline_property_bytes_span(owner, bucket, 0, value_slab_slots)?
        } else {
            Vec::new()
        };
        ...
        let emit = |slot: u32,
                    ...
                    visit: &mut dyn FnMut(
            BucketEntryPosition,
            EdgeWithInlineProperty<E>,
        ) -> ControlFlow<B>|
         -> Result<ControlFlow<B>, LabeledOperationError> {
            let inline_property = if width == 0 {
                InlinePropertyBytes::empty()
            } else if ordinal < value_slab_slots {
                let start = ordinal as usize * usize::from(width);
                let end = start + usize::from(width);
                InlinePropertyBytes::from_bytes(width, &value_prefix_bytes[start..end])
            } else {
                let bytes = self.read_bucket_inline_property_bytes_for_slot(
                    owner,
                    bucket,
                    ordinal,
                    log_chains.as_ref(),
                )?;
                InlinePropertyBytes::from_bytes(width, &bytes)
            };
            Ok(
                match visit(
                    BucketEntryPosition::new(slot),
        ...
        match order {
            OutEdgeOrder::Ascending => {
                // Dense value-slab prefix.
                for slot in 0..value_slab_slots {
                    let off = slot as usize * E::BYTES;
                    let edge = E::read_from(&raw_edges[off..off + E::BYTES])
                        .with_slot_index(slot)
                        .with_label_id(label.raw());
                    ...
                    if let ControlFlow::Break(value) =
                        emit(slot, next_ordinal, edge, &mut |slot, item| {
                            visit(slot, item)
                        })?
                    {
                        return Ok(ControlFlow::Break(value));
                ...
                // Edge-slab entries whose inline-property bytes live in the overflow log.
                if value_slab_slots < slab_slots {
                    for slot in value_slab_slots..slab_slots {
                        let off = slot as usize * E::BYTES;
                        let edge = E::read_from(&raw_edges[off..off + E::BYTES])
                            .with_slot_index(slot)
                            .with_label_id(label.raw());
                ...
                if bucket.overflow_log_head() >= 0 {
                        ...
                        .enumerate()
                    {
                        ...
                        let slot = slab_slots
                            + u32::try_from(log_pos).map_err(|_| {
                                LabeledOperationError::from(
                                    LaraOperationError::CollectAllocationOverflow,
                                )
                            })?;
                        let edge = edge.with_slot_index(slot).with_label_id(label.raw());
                        if edge.is_deleted_slot() || edge.is_tombstone_edge() {
                            continue;
                        }
                        if let ControlFlow::Break(value) =
            ...
            OutEdgeOrder::Descending => {
                // Need total live count to walk ordinals in reverse. Since reserved == degree,
                // every reserved slot is live; count them up front.
                let mut live_count = 0u32;
                for slot in 0..slab_slots {
                    let off = slot as usize * E::BYTES;
                    let edge = E::read_from(&raw_edges[off..off + E::BYTES]);
                    if !edge.is_deleted_slot() && !edge.is_tombstone_edge() {
                        live_count += 1;
                ...
                next_ordinal = live_count;

                // Overflow-log entries first (highest logical slots).
                if bucket.overflow_log_head() >= 0 {
                    let leaf = self.inline_property_bytes_log_leaf(owner);
                    let _chain = self.bucket_inline_property_bytes_log_chain(owner, bucket);
                    let log_indices: Vec<u32> = self
                        .edges
                        .overflow_log_chain_asc_indices(leaf, bucket.overflow_log_head());
                    for (log_pos, &entry_idx) in log_indices.iter().enumerate().rev() {
                        let (_, edge) = self.edges.read_overflow_log_entry(leaf, entry_idx);
                        let slot = slab_slots
                            + u32::try_from(log_pos).map_err(|_| {
                                LabeledOperationError::from(
                ...
                // Edge-slab entries whose inline-property bytes live in the overflow log.
                if value_slab_slots < slab_slots {
                    for slot in (value_slab_slots..slab_slots).rev() {
                        let off = slot as usize * E::BYTES;
                        let edge = E::read_from(&raw_edges[off..off + E::BYTES])
                            .with_slot_index(slot)
                            .with_label_id(label.raw());
                ...
                // Dense value-slab prefix.
                for slot in (0..value_slab_slots).rev() {
                    let off = slot as usize * E::BYTES;
                    let edge = E::read_from(&raw_edges[off..off + E::BYTES])
                        .with_slot_index(slot)
                        .with_label_id(label.raw());
