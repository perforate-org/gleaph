//! Experimental size-binned free span store with pluggable start-order index.
//!
//! The default `by_start` index uses
//! [`StablePagedOrderedMap`](ic_stable_paged_ordered_map::StablePagedOrderedMap) while keeping the
//! same record slab and size-class bins. [`FreeSpanBinnedBTreeStore`] keeps the previous
//! [`StableBTreeMap`]-backed index for benchmark comparison.
...
use super::{FreeSpan, dual_index::StartKey};
use crate::types::Address;

pub const MAGIC: [u8; 3] = *b"LFB";
...
impl fmt::Display for FreeSpanBinnedError {
...
impl std::error::Error for FreeSpanBinnedError {}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::BadMagic { actual } => {
                write!(
                    f,
                    "bad binned free span magic {actual:?}, expected {MAGIC:?}"
                )
            }
            Self::IncompatibleVersion(v) => write!(f, "unsupported binned layout version {v}"),
            Self::OutOfMemory => write!(f, "failed to allocate binned free span metadata"),
        }
    }
}
...
pub trait StartIndex {
    type Memory: Memory;

    fn init(memory: Self::Memory) -> Self;
    fn into_memory(self) -> Self::Memory;
    fn get(&self, start_slot: u64) -> Option<SpanId>;
    fn contains_key(&self, start_slot: u64) -> bool {
        self.get(start_slot).is_some()
    }
    fn insert(&mut self, start_slot: u64, id: SpanId);
    fn remove(&mut self, start_slot: u64) -> Option<SpanId>;
    fn predecessor(&self, start_slot: u64) -> Option<(u64, SpanId)>;
    fn successor(&self, start_slot: u64) -> Option<(u64, SpanId)>;
}

pub struct BTreeStartIndex<M: Memory>(StableBTreeMap<StartKey, u64, M>);

impl<M: Memory> StartIndex for BTreeStartIndex<M> {
    type Memory = M;
    ...
    fn get(&self, start_slot: u64) -> Option<SpanId> {
        self.0.get(&StartKey::new(start_slot))
    }

    fn contains_key(&self, start_slot: u64) -> bool {
        self.0.contains_key(&StartKey::new(start_slot))
    }

    fn insert(&mut self, start_slot: u64, id: SpanId) {
        self.0.insert(StartKey::new(start_slot), id);
    }

    fn remove(&mut self, start_slot: u64) -> Option<SpanId> {
        self.0.remove(&StartKey::new(start_slot))
    }
...
pub struct PagedStartIndex<M: Memory>(StablePagedOrderedMap<M>);

impl<M: Memory> StartIndex for PagedStartIndex<M> {
    type Memory = M;

    fn init(memory: Self::Memory) -> Self {
        Self(StablePagedOrderedMap::init(memory).expect("paged by_start index init failed"))
    }
    ...
    fn get(&self, start_slot: u64) -> Option<SpanId> {
        self.0.get(start_slot)
    }

    fn insert(&mut self, start_slot: u64, id: SpanId) {
        self.0
            .insert(start_slot, id)
    ...
    fn predecessor(&self, start_slot: u64) -> Option<(u64, SpanId)> {
        self.0.predecessor(start_slot)
    }

    fn successor(&self, start_slot: u64) -> Option<(u64, SpanId)> {
        self.0.successor(start_slot)
    }
}

pub type FreeSpanBinnedStore<M, MS> = FreeSpanBinnedStoreWith<M, PagedStartIndex<MS>>;

pub type FreeSpanBinnedBTreeStore<M, MS> = FreeSpanBinnedStoreWith<M, BTreeStartIndex<MS>>;

pub type FreeSpanBinnedPagedStore<M, MS> = FreeSpanBinnedStoreWith<M, PagedStartIndex<MS>>;

pub struct FreeSpanBinnedStoreWith<M: Memory, I: StartIndex> {
    store: M,
    by_start: RefCell<I>,
}

impl<M: Memory, I: StartIndex> FreeSpanBinnedStoreWith<M, I> {
    pub fn init(store: M, by_start_ms: I::Memory) -> Result<Self, InitError> {
        if store.size() == 0 {
            Self::write_fresh_header(&store)?;
        } else {
            Self::verify_header(&store)?;
        }
        Ok(Self {
            store,
            by_start: RefCell::new(I::init(by_start_ms)),
        })
    }

    fn write_fresh_header(mm: &M) -> Result<(), InitError> {
        crate::safe_write(mm, OFFSET_MAGIC, &MAGIC).map_err(|_| InitError::OutOfMemory)?;
        crate::safe_write(mm, OFFSET_VERSION, &[LAYOUT_VERSION])
            .map_err(|_| InitError::OutOfMemory)?;
        crate::safe_write(mm, 4, &[0; 4]).map_err(|_| InitError::OutOfMemory)?;
        crate::write_u64(mm, Address::from(OFFSET_RECORD_SLOTS), 0);
        crate::write_u64(mm, Address::from(OFFSET_ACTIVE_COUNT), 0);
        crate::write_u64(mm, Address::from(OFFSET_FREE_HEAD), 0);
        let zeros = [0u8; BIN_COUNT * 8];
        crate::safe_write(mm, OFFSET_BIN_HEADS, &zeros).map_err(|_| InitError::OutOfMemory)?;
        Ok(())
    }

    fn verify_header(mm: &M) -> Result<(), InitError> {
        let mut m = [0u8; 3];
        mm.read(OFFSET_MAGIC, &mut m);
        if m != MAGIC {
            return Err(InitError::BadMagic { actual: m });
        }
        let mut v = [0u8; 1];
        mm.read(OFFSET_VERSION, &mut v);
        if v[0] != LAYOUT_VERSION {
            return Err(InitError::IncompatibleVersion(v[0]));
        }
        Ok(())
    }

    pub fn into_memories(self) -> (M, I::Memory) {
    ...
    pub fn validate(&self) -> Result<(), FreeSpanBinnedError> {
        ...
        for bin in 0..BIN_COUNT {
            ...
            while cur != 0 {
                if cur > slots || !reached.insert(cur) {
                    return Err(FreeSpanBinnedError::BinInvariant);
                }
    ...
    pub fn release(&self, span: FreeSpan) -> Result<(), FreeSpanBinnedError> {
        if span.len == 0 {
            return Err(FreeSpanBinnedError::EmptySpan);
        }
        let span_end = span
            .start_slot
            .checked_add(span.len)
            .ok_or(FreeSpanBinnedError::SpanOverflow { span })?;
        if self.by_start.borrow().contains_key(span.start_slot) {
            return Err(FreeSpanBinnedError::DuplicateStart {
        ...
        if let Some(p) = prev {
            let end = p
                .start_slot
                .checked_add(p.len)
                .ok_or(FreeSpanBinnedError::SpanOverflow { span: p })?;
            if end > span.start_slot {
                return Err(FreeSpanBinnedError::OverlapPrevious {
                    previous: p,
                    inserted: span,
                });
            }
        ...
        if let Some(n) = next {
            if span_end > n.start_slot {
                return Err(FreeSpanBinnedError::OverlapNext {
                    inserted: span,
                    next: n,
                });
            }
    ...
    pub fn release_span(&self, start_slot: u64, len: u64) -> Result<(), FreeSpanBinnedError> {
    ...
    pub fn restore_allocated_prefix(&self, span: FreeSpan) -> Result<(), FreeSpanBinnedError> {
        if span.len == 0 {
            return Err(FreeSpanBinnedError::EmptySpan);
        }
        let span_end = span
    ...
    pub fn take_best_fit_whole(
        &self,
        min_len: u64,
    ) -> Result<Option<FreeSpan>, FreeSpanBinnedError> {
        if min_len == 0 {
            return Ok(None);
        }
    ...
    pub fn take_best_fit(&self, min_len: u64) -> Result<Option<FreeSpan>, FreeSpanBinnedError> {
        if min_len == 0 {
            return Ok(None);
        }
        let start_bin = size_class(min_len) as usize;
        for bin in start_bin..BIN_COUNT {
            ...
            if whole.len == min_len {
                ...
                self.by_start.borrow_mut().remove(whole.start_slot);
                let remainder_start = whole
                    .start_slot
                    .checked_add(min_len)
                    .ok_or(FreeSpanBinnedError::SpanOverflow { span: taken })?;
                let remainder = FreeSpan {
                    start_slot: remainder_start,
                    len: whole.len - min_len,
                };
                self.by_start.borrow_mut().insert(remainder.start_slot, id);
    ...
    pub fn replace_exact_pair_with(
        ...
        replacement
            ...
            .ok_or(FreeSpanBinnedError::SpanOverflow { span: replacement })?;
    ...
    fn ensure_span_exact(&self, span: FreeSpan) -> Result<(), FreeSpanBinnedError> {
        match self.free_span_starting_at(span.start_slot) {
            Some(found) if found == span => Ok(()),
            _ => Err(FreeSpanBinnedError::MissingSpan { span }),
        }
    }
    ...
    fn ensure_replacement_fits_after_removing(
        ...
        if let Some(p) = prev {
            ...
            let prev_end = p
                .start_slot
                .checked_add(p.len)
                .ok_or(FreeSpanBinnedError::SpanOverflow { span: previous })?;
    ...
    fn pick_span_in_bin(
        ...
        bin_idx: u32,
        min_len: u64,
    ) -> Result<Option<(SpanId, FreeSpan)>, FreeSpanBinnedError> {
    ...
    fn insert_span(&self, span: FreeSpan) -> Result<(), FreeSpanBinnedError> {
        let id = self.alloc_record()?;
        self.write_active_record(id, span)
    }

    fn write_active_record(&self, id: SpanId, span: FreeSpan) -> Result<(), FreeSpanBinnedError> {
        ...
        self.by_start.borrow_mut().insert(span.start_slot, id);
        self.inc_active(1)?;
        Ok(())
    }

    fn relink_active_record(&self, id: SpanId, span: FreeSpan) -> Result<(), FreeSpanBinnedError> {
        let bin_idx = size_class(span.len) as u8;
        let b = bin_idx as usize;
        let head = self.read_bin_head(b);
        self.write_record(
    ...
    fn replace_existing_span(
        ...
        let id = self
            ...
            .borrow()
            .get(old.start_slot)
            .ok_or(FreeSpanBinnedError::MissingSpan { span: old })?;
        let rec = self.read_record(id);
        if rec.flags != FLAG_ACTIVE || rec.len != old.len || rec.start_slot != old.start_slot {
            return Err(FreeSpanBinnedError::MissingSpan { span: old });
        }
        self.unlink_from_bin(id, rec)?;
        if old.start_slot != new.start_slot {
            self.by_start.borrow_mut().remove(old.start_slot);
            self.by_start.borrow_mut().insert(new.start_slot, id);
        }
        self.relink_active_record(id, new)
    }

    fn remove_span_exact(&self, span: FreeSpan) -> Result<(), FreeSpanBinnedError> {
        let id = self
            .by_start
            .borrow_mut()
            .remove(span.start_slot)
            .ok_or(FreeSpanBinnedError::MissingSpan { span })?;
        let rec = self.read_record(id);
        if rec.flags != FLAG_ACTIVE || rec.len != span.len || rec.start_slot != span.start_slot {
            self.by_start.borrow_mut().insert(span.start_slot, id);
            return Err(FreeSpanBinnedError::MissingSpan { span });
        }
        self.unlink_from_bin(id, rec)?;
        self.free_record(id)?;
        self.inc_active(-1)?;
    ...
    fn unlink_from_bin(&self, id: SpanId, rec: SpanRecord) -> Result<(), FreeSpanBinnedError> {
    ...
    fn alloc_record(&self) -> Result<SpanId, FreeSpanBinnedError> {
        ...
        if fh != 0 {
            ...
            if last_byte > 0 {
                crate::safe_write(&self.store, last_byte - 1, &[0])
                    .map_err(|_| FreeSpanBinnedError::CorruptedFreeList)?;
            }
    ...
    fn free_record(&self, id: SpanId) -> Result<(), FreeSpanBinnedError> {
    ...
    fn write_record(&self, rec: SpanRecord, id: SpanId) -> Result<(), FreeSpanBinnedError> {
        let off = record_offset(id);
        crate::write_u64(
            &self.store,
    ...
    fn write_record_fields<F>(&self, id: SpanId, f: F) -> Result<(), FreeSpanBinnedError>
    where
    ...
    fn write_bin_head(&self, bin: usize, head: SpanId) -> Result<(), FreeSpanBinnedError> {
        crate::write_u64(
            &self.store,
            Address::from(OFFSET_BIN_HEADS + bin as u64 * 8),
            head,
    ...
    fn write_record_slots(&self, slots: u64) -> Result<(), FreeSpanBinnedError> {
    ...
    fn inc_active(&self, delta: i64) -> Result<(), FreeSpanBinnedError> {
    ...
    fn write_free_head(&self, h: SpanId) -> Result<(), FreeSpanBinnedError> {
...
mod tests {
    ...
    -> FreeSpanBinnedStore<VirtualMemory<DefaultMemoryImpl>, VirtualMemory<DefaultMemoryImpl>> {
        ...
        FreeSpanBinnedStore::init(m.get(MemoryId::new(10)), m.get(MemoryId::new(11))).unwrap()
    }

    fn test_paged_store()
    -> FreeSpanBinnedPagedStore<VirtualMemory<DefaultMemoryImpl>, VirtualMemory<DefaultMemoryImpl>>
    {
        let m = MemoryManager::init(DefaultMemoryImpl::default());
        FreeSpanBinnedPagedStore::init(m.get(MemoryId::new(40)), m.get(MemoryId::new(41))).unwrap()
    }
    ...
    fn binned_paged_take_split_restore_and_reopen() {
        ...
        let s = FreeSpanBinnedPagedStore::init(meta, bs).unwrap();
        s.release_span(1000, 80).unwrap();
        s.release_span(2000, 32).unwrap();
        let span = s.take_best_fit(40).unwrap().unwrap();
        assert_eq!(
        ...
        let reopened = FreeSpanBinnedPagedStore::init(meta, bs).unwrap();
        assert_eq!(
    ...
    fn binned_reopen_preserves_state() {
        ...
        let s = FreeSpanBinnedStore::init(meta, bs).unwrap();
        s.release_span(42, 100).unwrap();
        s.release_span(500, 25).unwrap();
        assert_eq!(s.len(), 2);
        let (meta, bs) = s.into_memories();
        let s2 = FreeSpanBinnedStore::init(meta, bs).unwrap();
        assert_eq!(s2.len(), 2);
        assert_eq!(
    ...
    fn binned_duplicate_start_rejected() {
        ...
        assert_eq!(
            s.release_span(1, 3),
            Err(FreeSpanBinnedError::DuplicateStart { start_slot: 1 })
        );
    }
    ...
    fn binned_overlap_next_rejected() {
        ...
        assert_eq!(
            ...
            Err(FreeSpanBinnedError::OverlapNext {
                inserted: FreeSpan {
                    start_slot: 25,
                    len: 10
                },
    ...
    fn binned_replace_pair_is_non_destructive_on_missing_right() {
        ...
        assert_eq!(
            err,
            FreeSpanBinnedError::MissingSpan {
                span: FreeSpan {
                    start_slot: 13,
                    len: 5,
                }
    ...
    fn binned_release_rejects_overflowing_span() {
        ...
        assert_eq!(
            s.release(span),
            Err(FreeSpanBinnedError::SpanOverflow { span })
        );
    }
}
