...
//! [`gleaph_gql_ic_wire`] and re-exported here under the historical `IcWire*` names so the
//! execution side (router/graph) and the SDK share one wire shape. This module additionally owns
...
pub use gleaph_gql_ic_wire::{
    GqlWireDecodeError as WireError, GqlWirePathElement as IcWirePathElement,
    GqlWireValue as IcWireValue,
};
