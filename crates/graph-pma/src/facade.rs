...
pub mod experimental_dgap {
    ...
    //! DGAP: `M_v`, two `M_e` regions, optional separate stream log.
    pub use crate::low_level::{
        DGAP_EDGES_AND_LOG_MEMORY_SLOT, DGAP_LOG_MEMORY_SLOT, DGAP_SEGMENT_EDGE_COUNTS_MEMORY_SLOT,
        DGAP_VERTEX_MEMORY_SLOT, GleaphMemoryManager,
    };
    use ic_stable_structures::memory_manager::MemoryId;

    /// `(M_v, M1 segment_edge_counts, M2 edges+log, M_l stream)`.
    #[inline]
    pub fn ic_stable_csr_memory_ids() -> (MemoryId, MemoryId, MemoryId, MemoryId) {
        (
            MemoryId::new(DGAP_VERTEX_MEMORY_SLOT),
            MemoryId::new(DGAP_SEGMENT_EDGE_COUNTS_MEMORY_SLOT),
