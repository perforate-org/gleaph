...
- `wasm32-unknown-unknown` target: the production/benchmark target (see `build.rs`; wasm32 has a prebuilt std, so no `-Z build-std` is needed); SIMD comes from the workspace-root `.cargo/config.toml` target rustflags
