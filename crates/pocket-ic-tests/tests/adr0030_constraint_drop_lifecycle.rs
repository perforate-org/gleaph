...
//! (`Removed`). These tests exercise the public ingress (Candid → `gql_execute`) and the
...
use gleaph_pocket_ic_tests::{
    ...
    gql_execute_as_admin, gql_execute_as_admin_expect_err,
    gql_execute_result_as_admin, gql_query_as_admin, graph_unique_outbox_len_all_shards,
...
fn drop_constraint_releases_committed_values() {
    ...
    let dropped =
        gql_execute_result_as_admin(&env, &drop_for(CONSTRAINT), "s9-rel-drop");
...
fn drop_then_recreate_same_name_different_label() {
    ...
    gql_execute_as_admin(&env, &insert(LABEL, PROPERTY, "a@example.com"), "s9-rc-ins");

    gql_execute_as_admin(&env, &drop_for(CONSTRAINT), "s9-rc-drop");
    ...
    gql_execute_as_admin(&env, &insert(OTHER_LABEL, "code", "x"), "s9-rc-new-ins");
    let dup = gql_execute_as_admin_expect_err(
        &env,
...
fn dropping_constraint_admits_new_inserts_but_blocks_recreate() {
    ...
    gql_execute_as_admin(&env, &insert(LABEL, PROPERTY, "a@example.com"), "s9-pt-ins");

    // Flip to Dropping but do **not** run recovery yet, so the constraint is still mid-drain.
    gql_execute_as_admin(&env, &drop_for(CONSTRAINT), "s9-pt-drop");
...
fn drop_during_in_flight_insert() {
    ...
    gql_execute_as_admin(&env, &drop_for(CONSTRAINT), "s9-if-drop");
...
fn recreate_blocked_until_pending_effect_drained() {
    ...
    gql_execute_as_admin(&env, &drop_for(CONSTRAINT), "s9-gate-drop");
    run_router_recovery_timer(&env);
    let blocked = gql_execute_as_admin_expect_err(
        &env,
        &create_for(CONSTRAINT, OTHER_LABEL, "code"),
...
fn drop_survives_upgrade() {
    ...
    gql_execute_as_admin(&env, &insert(LABEL, PROPERTY, "a@example.com"), "s9-up-ins");

    gql_execute_as_admin(&env, &drop_for(CONSTRAINT), "s9-up-drop");
...
fn drop_does_not_disable_unrelated_constraints() {
    ...
    gql_execute_as_admin(&env, &insert(OTHER_LABEL, "code", "m1"), "s9-un-ins-2");

    // Drop only the user_email constraint.
    gql_execute_as_admin(&env, &drop_for(CONSTRAINT), "s9-un-drop");
    run_router_recovery_timer(&env);
