...
use gleaph_pocket_ic_tests::{
    GRAPH_HOME_NAME, e2e_insert_vertex, gql_execute_as_admin,
    gql_execute_as_admin_expect_err, gql_query_as_admin, gql_query_as_admin_expect_err,
...
fn catalog_create_graph_type_returns_zero_rows() {
    ...
    let row_count =
        gql_execute_as_admin(&env, &ddl, "catalog_create_graph_type_returns_zero_rows");
...
fn catalog_typed_binding_persists_across_calls() {
    ...
    gql_execute_as_admin(&env, &type_ddl, "catalog_typed_binding_type");
    let bind_ddl = format!("CREATE GRAPH {GRAPH_HOME_NAME} TYPED gt");
    gql_execute_as_admin(&env, &bind_ddl, "catalog_typed_binding_graph");
...
fn catalog_typed_schema_rejects_undirected_match_on_directed_edge() {
    ...
    gql_execute_as_admin(&env, &setup, "catalog_schema_setup");
...
fn catalog_create_graph_unregistered_name_rejected() {
    let env = install_two_graph_federation();
    let type_ddl = format!("CREATE GRAPH TYPE gt {{ {PERSON_KNOWS} }}");
    gql_execute_as_admin(&env, &type_ddl, "catalog_unregistered_type");
    let bind_ddl = "CREATE GRAPH missing_graph TYPED gt";
    let err =
        gql_execute_as_admin_expect_err(&env, bind_ddl, "catalog_unregistered_bind");
    assert!(
        matches!(
            err,
...
fn catalog_drop_graph_type_cascades_typed_binding() {
    ...
    gql_execute_as_admin(&env, &setup, "catalog_drop_setup");
    gql_execute_as_admin(&env, "DROP GRAPH TYPE gt", "catalog_drop_type");

    let bind_again = format!("CREATE GRAPH {GRAPH_HOME_NAME} TYPED gt");
    let err = gql_execute_as_admin_expect_err(&env, &bind_again, "catalog_drop_rebind");
    assert!(
        matches!(
            err,
            gleaph_graph_kernel::federation::RouterError::NotFound(_)
