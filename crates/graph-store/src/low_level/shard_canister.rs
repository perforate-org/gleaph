...
//! [`SHARD_CANISTER_DIRECTORY_MAGIC`]). The [`crate::facade::GraphStore`] stores this payload in
//! stable memory under [`super::region::RegionKind::ShardCanisterDirectory`] (extent-backed), and
//! writes it on [`crate::facade::GraphStore::try_write_all_to_stable_memory`] /
//! [`crate::facade::GraphStore::try_refresh_and_write_dirty_to_stable_memory`].
//!
//! # Hydration consistency (strict)
//!
//! After loading the directory, [`crate::low_level::GraphRuntime::validate_shard_canister_slots`]
//! runs during [`crate::facade::GraphStore::hydrate_from_stable_memory`]: every **live**
...
type ShardDirectoryMap<M> = StableBTreeMap<ShardCanisterSlot, Principal, GraphAdjacencyMemory<M>>;

pub struct ShardDirectoryStore<M: Memory + Clone> {
    entries: ShardDirectoryMap<M>,
}
...
impl<M: Memory + Clone> ShardDirectoryStore<M> {
    pub fn open(slots: &GraphStoreMemorySlots<M>) -> Self {
        Self {
            entries: StableBTreeMap::init(slots.shard_canister_directory()),
        }
    }
    ...
    pub fn push_principal(
        ...
        if reject_duplicates && self.entries.iter().any(|entry| entry.value() == principal) {
            return None;
        }
        let slot = u16::try_from(self.entries.len()).ok()?;
        self.entries.insert(slot, principal);
        Some(slot)
    }

    pub fn to_directory(&self) -> ShardCanisterDirectory {
        let mut directory = ShardCanisterDirectory::default();
        for entry in self.entries.iter() {
            let slot = *entry.key() as usize;
            while directory.principals.len() < slot {
                directory.principals.push(Principal::anonymous());
            }
            if directory.principals.len() == slot {
                directory.principals.push(entry.value());
            } else {
                directory.principals[slot] = entry.value();
            }
        }
        directory
    }

    pub fn replace_from_directory(&mut self, directory: &ShardCanisterDirectory) {
        let keys: Vec<ShardCanisterSlot> = self.entries.iter().map(|entry| *entry.key()).collect();
        for key in keys {
            self.entries.remove(&key);
        }
        for (slot, principal) in directory.principals.iter().copied().enumerate() {
            let slot = u16::try_from(slot).expect("shard directory slot fits u16");
            self.entries.insert(slot, principal);
        }
    }
}
...
mod tests {
    use super::{
        SHARD_CANISTER_DIRECTORY_MAGIC, ShardCanisterDirectory, ShardDirectoryStore,
    };
    use crate::GraphStoreMemorySlots;
    use candid::Principal;
    ...
    #[test]
    fn shard_directory_store_round_trips_via_fixed_slot_map() {
        let slots = GraphStoreMemorySlots::new(VectorMemory::default());
        let mut store = ShardDirectoryStore::open(&slots);
        let p0 = Principal::anonymous();
        let p1 = Principal::from_slice(&[1, 2, 3, 4, 5, 6, 7, 8, 9]);
        assert_eq!(store.push_principal(p0, true), Some(0));
        assert_eq!(store.push_principal(p1, true), Some(1));
        assert_eq!(store.push_principal(p1, true), None);

        let reopened = ShardDirectoryStore::open(&slots);
        assert_eq!(reopened.principal(0), Some(p0));
        assert_eq!(reopened.principal(1), Some(p1));
        assert_eq!(reopened.to_directory().len(), 2);
    }
}
