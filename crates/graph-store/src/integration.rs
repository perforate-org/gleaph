//! Integration helpers for upper layers that use the graph-store facade.
//!
//! The low-level APIs are intentionally explicit, but upper layers often want
//! a slightly more declarative entrypoint for initial graph bootstrap. This
//! module keeps that convenience separate from the core facade/runtime types.
...
pub use label_index::{
    LabelCatalogStore, LabelMembership, VERTEX_LABEL_PROMOTION_THRESHOLD_BASE, VacuumStats,
    VertexGcState, VertexLabelIndex, VertexLabelStateStore, decode_label_catalog_metadata,
    decode_vertex_label_catalog, decode_vertex_label_gc_state, encode_label_catalog_metadata,
    encode_vertex_label_catalog, encode_vertex_label_gc_state, decode_vertex_label_index_blob,
    encode_vertex_label_index_blob,
    load_vertex_label_catalog_from_slots, store_vertex_label_catalog_to_slots,
};
pub use overlay_types::{
    ExpansionWithShard, GraphStoreKernelOverlayObservability, GraphStoreOverlayBootstrapGraphSummary,
    GraphStoreOverlayEdgeBootstrapSummary, GraphStoreOverlayEdgeMutationKind,
    GraphStoreOverlayEdgeWriteSummary, GraphStoreOverlayInsertEdgeSummary,
    GraphStoreOverlayNodeBootstrapSummary, GraphStoreOverlayNodeDeleteSummary,
    GraphStoreOverlayWriteEvent,
};
...
pub fn bootstrap_kernel_overlay_graph<S: GraphStoreStore>(
    graph: &mut GraphStoreKernelOverlayGraph<'_, S>,
    ...
    for node in &spec.nodes {
        ...
        if let Some(GraphStoreOverlayWriteEvent::BootstrapNode(summary)) =
    ...
    for edge in &spec.edges {
        ...
        edges.push(record);
        if let Some(GraphStoreOverlayWriteEvent::BootstrapEdge(summary)) =
            graph.write_history().last()
        {
    ...
    let overlay_summary = GraphStoreOverlayBootstrapGraphSummary::from_bootstrap_summaries(
        &node_summaries,
        &edge_summaries,
        locators,
    );
...
/// Bootstrap-oriented bridge from graph service calls to kernel records.
///
...
/// terms of `NodeRecord` / `EdgeRecord` while the storage layer is still
/// growing into the full `GraphRead` / `GraphWrite` surface.
pub struct GraphStoreKernelBootstrapBridge<'a, S: GraphStoreStore> {
    store: S,
    memory: &'a S::Mem,
    next_node_id: u64,
    next_edge_id: u64,
    ...
    vertex_ordinals: Vec<GraphStoreVertexOrdinalMapping>,
    semantic_node_id_by_forward_ordinal: Vec<Option<NodeId>>,
    vertex_ordinal_by_node_id: BTreeMap<NodeId, GraphStoreVertexOrdinalMapping>,
    vertex_label_index: VertexLabelIndex,
    vertex_gc_state: VertexGcState,
    edge_locators: BTreeMap<EdgeId, GraphStoreEdgeLogicalLocatorMapping>,
    forward_base_slots_by_ordinal: Vec<Vec<Option<EdgeId>>>,
    reverse_base_slots_by_ordinal: Vec<Vec<Option<EdgeId>>>,
    last_property_write_summary: Option<GraphStorePropertyMutationWriteSummary>,
    last_insert_edge_summary: Option<GraphStoreOverlayInsertEdgeSummary>,
    last_edge_write_summary: Option<GraphStoreOverlayEdgeWriteSummary>,
    last_node_delete_summary: Option<GraphStoreOverlayNodeDeleteSummary>,
    property_write_history: Vec<GraphStorePropertyMutationWriteSummary>,
    insert_edge_history: Vec<GraphStoreOverlayInsertEdgeSummary>,
    edge_write_history: Vec<GraphStoreOverlayEdgeWriteSummary>,
    node_delete_history: Vec<GraphStoreOverlayNodeDeleteSummary>,
    write_history: Vec<GraphStoreOverlayWriteEvent>,
}
...
/// through the graph facade, while the kernel-facing record layer is
/// currently maintained in-memory and treated as authoritative for
/// `GraphRead` / `GraphWrite`. That keeps the boundary usable while storage
...
/// Small owned harness that keeps the facade and its stable memory together.
///
...
impl<M: Memory + Clone> GraphStoreKernelHarness<M> {
    ...
    pub fn bootstrap_empty(memory: M) -> GraphStoreResult<Self> {
        ...
        let facade = GraphStore::bootstrap_empty_with_bucket_size_using_memory_rc(
    ...
    pub fn facade(&self) -> &GraphStore<M> {
        &self.facade
    }

    /// Returns mutable access to the facade stored inside this harness.
    pub fn facade_mut(&mut self) -> &mut GraphStore<M> {
        &mut self.facade
    }

    /// Binds the owned facade and memory as one kernel-facing overlay graph.
    pub fn bind_overlay(&mut self) -> GraphStoreKernelHarnessOverlay<'_, M> {
        GraphStoreKernelHarnessOverlay::new(&mut self.facade, self.memory.as_ref())
    }
    ...
    pub fn bind_overlay_with_graph(
        ...
        GraphStoreKernelHarnessOverlay<'_, M>,
    ...
    /// Applies one declarative bootstrap specification through the owned facade.
    pub fn bootstrap_graph(
        &mut self,
        spec: &BootstrapGraphSpec,
    ) -> GraphStoreResult<GraphStoreBootstrapGraphWriteSummary> {
    ...
    /// Bootstraps packed vertex refs and logical edges directly through the owned facade.
...
impl<'a, S: GraphStoreStore> GraphStats for GraphStoreKernelOverlayGraph<'a, S> {
    fn label_cardinality(&self, label: &str) -> Option<u64> {
...
impl<'a, S: GraphStoreStore> GraphStoreKernelOverlayGraph<'a, S> {
    fn edge_base_logical_index(slots: &[Option<EdgeId>], edge_id: EdgeId) -> Option<usize> {
        GraphStoreKernelBootstrapBridge::<S>::find_base_logical_index(slots, edge_id)
    }
...
mod tests {
    ...
    use crate::facade::{
        GraphStoreBootstrapVerticesProjection, GraphStoreEdgeWriteOperation, GraphStoreRefreshedVertices,
        GraphStoreVertexOrdinalMapping, GraphStoreWriteEventProjection,
    };
    use crate::integration::{
        GraphStoreKernelOverlayObservability, GraphStoreOverlayEdgeMutationKind,
        GraphStoreOverlayWriteEvent,
    };
    use crate::low_level::{
        ...
        OverflowEntry, RegionKind, ShardDirectoryStore, SurfaceBaseStorage, SurfaceKind, VertexEntry,
        VertexLabelIndexEntry,
    };
    use crate::observability::{
        GraphStoreDiagnosticsView, project_overlay_write_event, project_overlay_write_history,
    };
    use crate::property_index::{PropertyIndexError, PropertyIndexNodeStoreMutationKind};
    use crate::property_store::PropertyStoreError;
    use crate::{GraphStore, GraphStoreError};
    ...
    fn assert_projected_history(
        events: &[GraphStoreOverlayWriteEvent],
    ...
    fn kernel_harness_can_bootstrap_graph_spec() {
        let mut harness = GraphStoreKernelHarness::bootstrap_empty(VecMemory::default())
    ...
    fn kernel_harness_can_bind_and_bootstrap_same_overlay_instance() {
        ...
        assert!(matches!(
            graph.write_history(),
            [
                GraphStoreOverlayWriteEvent::BootstrapNode(_),
                GraphStoreOverlayWriteEvent::BootstrapNode(_),
                GraphStoreOverlayWriteEvent::BootstrapEdge(_),
                GraphStoreOverlayWriteEvent::BootstrapGraph(summary)
        ...
        let expected_bootstrap = crate::observability::format_write_event_projection(
            &GraphStoreWriteEventProjection::BootstrapGraph(summary.projection()),
        );
        assert_eq!(
        ...
        assert_eq!(
            GraphStoreDiagnosticsView::debug_report(&graph)
                .lines()
                .last()
                .map(str::to_owned),
    ...
    fn kernel_overlay_bootstrap_function_can_seed_graph_records() {
        ...
        assert!(matches!(
            ...
            [
                ...
                GraphStoreOverlayWriteEvent::BootstrapNode(_),
                GraphStoreOverlayWriteEvent::BootstrapEdge(_),
                GraphStoreOverlayWriteEvent::BootstrapGraph(graph_summary)
        ...
        let overlay_projection = match overlay.write_history().last() {
            Some(GraphStoreOverlayWriteEvent::BootstrapGraph(graph_summary)) => {
    ...
    fn kernel_bootstrap_bridge_can_create_node_and_edge_records() {
        let memory = VecMemory::default();
        let mut facade = GraphStore::bootstrap_empty(memory.clone()).expect("bootstrap");
        let mut bridge = GraphStoreKernelBootstrapBridge::new(&mut facade, &memory);
    ...
    fn kernel_overlay_inserts_shard_canister_dst_edge() {
        ...
        let hydrated = GraphStore::hydrate_from_stable_memory(manager, mem).expect("hydrate");
        assert_eq!(hydrated.shard_canister_directory.principal(0), Some(remote));
    }

    /// [`GraphStore::hydrate_from_stable_memory`] must reject a shard directory that is empty
    ...
    fn hydrate_rejects_truncated_shard_directory_against_live_shard_edges() {
        ...
        let slots = GraphStoreMemorySlots::for_root_memory(BorrowedMemory::new(mem_rc.as_ref()));
        let mut shard_store = ShardDirectoryStore::open(&slots);
        shard_store.replace_from_directory(&crate::low_level::ShardCanisterDirectory::default());

        let manager = facade.manager.borrow().clone();
        let mem = (*mem_rc).clone();
        let err = GraphStore::hydrate_from_stable_memory(manager, mem).expect_err("hydrate");
        assert_eq!(
            err,
            GraphStoreError::Hydration(HydrationError::ShardCanisterSlotOutOfRange {
                slot: 0,
                directory_len: 0,
            })
        );
    ...
    fn kernel_overlay_graph_exposes_property_write_summary() {
        ...
        assert_projected_history(
            graph.write_history(),
            vec![
                GraphStoreWriteEventProjection::BootstrapVertices(
                    GraphStoreBootstrapVerticesProjection {
                        ordinals: vec![(0, 0)],
                        refreshed: GraphStoreRefreshedVertices::new(Vec::new(), Vec::new()),
                    },
                ),
                GraphStoreWriteEventProjection::Property(node_property_projection.clone()),
            ],
        );
        ...
        assert!(matches!(
            ...
            [
                GraphStoreWriteEventProjection::BootstrapVertices(_),
                GraphStoreWriteEventProjection::Property(_),
                GraphStoreWriteEventProjection::InsertEdge(_),
                GraphStoreWriteEventProjection::Property(_)
            ]
        ));
        assert_eq!(
            shared_history[1],
            GraphStoreWriteEventProjection::Property(node_property_projection)
        );
        assert_eq!(
            shared_history[3],
            GraphStoreWriteEventProjection::Property(edge_property_projection)
        );
        assert_eq!(
        ...
        assert!(matches!(
            ...
            [
                GraphStoreOverlayWriteEvent::BootstrapNode(_),
                GraphStoreOverlayWriteEvent::Property(_),
                GraphStoreOverlayWriteEvent::InsertEdge(_),
                GraphStoreOverlayWriteEvent::Property(_)
            ]
        ));
        let shared_history = graph.shared_write_history();
    ...
    fn kernel_overlay_graph_insert_summary_exposes_rebalance_projection() {
        ...
        *facade.manager.borrow_mut() = manager;
        ...
        assert!(matches!(
            ...
            [
                ..,
                GraphStoreWriteEventProjection::EnsureCapacity(_),
                GraphStoreWriteEventProjection::InsertEdge(_)
            ]
        ));
        assert_eq!(
            shared_history.last(),
            Some(&GraphStoreWriteEventProjection::InsertEdge(
                summary.projection()
            ))
        );
        assert!(matches!(
            graph.write_history().last(),
            Some(GraphStoreOverlayWriteEvent::InsertEdge(_))
        ));
        assert_eq!(
            project_overlay_write_event(graph.write_history().last().expect("last overlay event")),
            vec![
                GraphStoreWriteEventProjection::EnsureCapacity(ensure.clone()),
                GraphStoreWriteEventProjection::InsertEdge(summary.projection()),
            ]
        );
        assert_eq!(
        ...
        let debug_report = GraphStoreDiagnosticsView::debug_report(&graph);
        assert!(debug_report.contains("ensure-capacity rebalanced=true"));
        assert!(debug_report.contains("insert-edge inserted=true"));
    }

    #[test]
    fn graph_pma_store_adapter_can_convert_into_kernel_overlay() {
        let memory = VecMemory::default();
        let mut facade = GraphStore::bootstrap_empty(memory.clone()).expect("bootstrap");
    ...
    fn kernel_overlay_graph_can_update_and_delete_edges() {
        ...
        assert_eq!(
            project_overlay_write_event(graph.write_history().last().expect("last overlay event")),
            vec![GraphStoreWriteEventProjection::Edge(label_projection.clone())]
        );
        ...
        assert_eq!(
            ...
            GraphStoreOverlayEdgeMutationKind::Delete
        );
        ...
        assert!(matches!(
            ...
            [
                ...
                GraphStoreOverlayWriteEvent::BootstrapNode(_),
                GraphStoreOverlayWriteEvent::InsertEdge(_),
                GraphStoreOverlayWriteEvent::Edge(_),
                GraphStoreOverlayWriteEvent::Edge(_)
            ]
        ));
        let expansions = graph
    ...
    fn kernel_overlay_graph_can_delete_node_with_detach() {
        ...
        let event_projection = graph
            .write_history()
            .iter()
            .find_map(GraphStoreOverlayWriteEvent::node_delete_projection)
            .expect("node delete event projection");
        assert_eq!(summary.projection(), event_projection);
        assert_eq!(
            project_overlay_write_event(graph.write_history().last().expect("last overlay event")),
            vec![GraphStoreWriteEventProjection::NodeDelete(
                summary.projection()
            )]
        );
        assert_eq!(
            graph.shared_write_history().last(),
            Some(&GraphStoreWriteEventProjection::NodeDelete(
                summary.projection()
            ))
        );
        assert_eq!(
        ...
        assert_eq!(
            GraphStoreKernelOverlayObservability::formatted_last_write_event(&graph),
            Some(
                "node-delete detached=true edges=1 deleted=[1] edge-writes=Delete@Base".to_owned()
            )
        );
        assert_eq!(
            GraphStoreDiagnosticsView::formatted_last_write_event(&graph),
            Some(
                "node-delete detached=true edges=1 deleted=[1] edge-writes=Delete@Base".to_owned()
            )
        );
        ...
        assert!(matches!(
            ...
            [
                ...
                GraphStoreOverlayWriteEvent::InsertEdge(_),
                GraphStoreOverlayWriteEvent::Edge(_),
                GraphStoreOverlayWriteEvent::NodeDelete(_)
            ]
        ));
    ...
    fn bench_profile_smoke_repeated_bootstrap_node() {
        ...
        let mut harness =
            GraphStoreKernelHarness::bootstrap_empty(VecMemory::default()).expect("harness");
        let mut graph = harness.bind_overlay();
        let labels = vec!["Person".to_owned()];
