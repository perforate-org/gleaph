...
use crate::facade::{
    GraphStore, GraphStoreEdgeWriteProjection, GraphStoreFacadeWriteEvent,
    GraphStoreMaintenanceQueueItemProjection, GraphStoreMaintenanceQueueStorageProjection,
    GraphStoreRefreshedVertices, GraphStoreStore, GraphStoreStoreAdapter, GraphStoreWriteEventProjection,
    PropertyIndexFallbackReason,
};
use crate::integration::{GraphStoreKernelOverlayGraph, GraphStoreOverlayWriteEvent};
...
pub trait GraphStoreDiagnosticsView {
...
fn format_edge_write_operation(summary: &GraphStoreEdgeWriteProjection) -> String {
    format!("{:?}@{:?}", summary.operation, summary.path)
}

fn format_edge_write_operations(summaries: &[GraphStoreEdgeWriteProjection]) -> String {
...
fn format_refreshed_vertices(summary: &GraphStoreRefreshedVertices) -> String {
    format!(
        "({},{}) fwd={} rev={}",
        summary.forward.len(),
        summary.reverse.len(),
...
pub fn format_write_event_projection(event: &GraphStoreWriteEventProjection) -> String {
    match event {
        GraphStoreWriteEventProjection::BootstrapVertices(summary) => {
            format!(
        ...
        GraphStoreWriteEventProjection::BootstrapEdge(summary) => {
            format!(
        ...
        GraphStoreWriteEventProjection::BootstrapGraph(summary) => {
            format!(
        ...
        GraphStoreWriteEventProjection::EnsureCapacity(summary) => {
            format!(
        ...
        GraphStoreWriteEventProjection::InsertEdge(summary) => {
            format!(
        ...
        GraphStoreWriteEventProjection::MaintenanceCycle(summary) => {
            format!(
        ...
        GraphStoreWriteEventProjection::MaintenanceBatch(summary) => {
            format!(
        ...
        GraphStoreWriteEventProjection::MaintenanceQueue(summary) => {
            format!(
        ...
        GraphStoreWriteEventProjection::Property(summary) => {
        ...
        GraphStoreWriteEventProjection::Edge(summary) => {
            format!(
        ...
        GraphStoreWriteEventProjection::NodeDelete(summary) => {
            format!(
...
pub fn format_write_event_history(events: &[GraphStoreWriteEventProjection]) -> Vec<String> {
...
pub fn format_last_write_event(events: &[GraphStoreWriteEventProjection]) -> Option<String> {
    events.last().map(format_write_event_projection)
}

pub fn format_maintenance_queue_item(item: &GraphStoreMaintenanceQueueItemProjection) -> String {
    format!(
...
pub fn format_maintenance_queue(items: &[GraphStoreMaintenanceQueueItemProjection]) -> Vec<String> {
    items.iter().map(format_maintenance_queue_item).collect()
}

pub fn format_maintenance_queue_storage(
    storage: &GraphStoreMaintenanceQueueStorageProjection,
) -> String {
    format!(
...
pub fn format_write_event_report(events: &[GraphStoreWriteEventProjection]) -> String {
...
pub fn project_facade_write_event(
    event: &GraphStoreFacadeWriteEvent,
) -> Vec<GraphStoreWriteEventProjection> {
...
pub fn last_projected_facade_event(
    events: &[GraphStoreFacadeWriteEvent],
) -> Option<GraphStoreWriteEventProjection> {
    events
        .iter()
        .rev()
        .find_map(GraphStoreFacadeWriteEvent::shared_projection)
}

/// Projects one overlay write event onto the shared event vocabulary.
pub fn project_overlay_write_event(
    event: &GraphStoreOverlayWriteEvent,
) -> Vec<GraphStoreWriteEventProjection> {
    event.shared_projections()
}
...
impl<T> GraphStoreDiagnosticsView for &T
where
    T: GraphStoreDiagnosticsView + ?Sized,
{
    fn shared_write_history(&self) -> Vec<GraphStoreWriteEventProjection> {
        (**self).shared_write_history()
    }
...
impl<'a, S: GraphStoreStore> GraphStoreDiagnosticsView for GraphStoreStoreAdapter<'a, S> {
    fn shared_write_history(&self) -> Vec<GraphStoreWriteEventProjection> {
        self.shared_write_history()
    }
}

impl<'a, S: GraphStoreStore> GraphStoreDiagnosticsView for GraphStoreKernelOverlayGraph<'a, S> {
    fn shared_write_history(&self) -> Vec<GraphStoreWriteEventProjection> {
        self.shared_write_history()
    }
}
...
mod tests {
    use super::{
        GraphStoreDiagnosticsView, format_last_write_event, format_maintenance_queue,
        format_maintenance_queue_storage, format_write_event_history,
        format_write_event_projection, format_write_event_report,
    };
    use crate::VecMemory;
    use crate::facade::{
        GraphStore, GraphStoreEdgeWriteOperation, GraphStoreEdgeWriteProjection,
        GraphStoreEnsureCapacityProjection, GraphStoreMaintenanceBatchProjection,
        ...
        GraphStoreMaintenanceQueueStorageProjection, GraphStoreNodeDeleteProjection,
        GraphStorePropertyIndexTouchedSections, GraphStorePropertyWriteProjection,
        GraphStoreRefreshedVertices, GraphStoreWriteEventProjection,
    };
    ...
    fn formatter_formats_ensure_capacity_projection() {
        let projection =
            GraphStoreWriteEventProjection::EnsureCapacity(GraphStoreEnsureCapacityProjection {
                rebalanced: true,
                total_displacement: 4,
                max_displacement: 2,
                refreshed: GraphStoreRefreshedVertices::new(vec![0, 1], vec![3]),
            });
    ...
    fn formatter_formats_maintenance_projections() {
        let cycle =
            GraphStoreWriteEventProjection::MaintenanceCycle(GraphStoreMaintenanceCycleProjection {
                ...
                total_displacement: 4,
                max_displacement: 2,
                refreshed: GraphStoreRefreshedVertices::new(vec![1], vec![2]),
        ...
        let batch =
            GraphStoreWriteEventProjection::MaintenanceBatch(GraphStoreMaintenanceBatchProjection {
                cycles: 2,
                queue_len_before: 5,
                queue_len_after: 1,
        ...
        let queue_update =
            GraphStoreWriteEventProjection::MaintenanceQueue(GraphStoreMaintenanceQueueProjection {
    ...
    fn formatter_formats_maintenance_queue() {
        let queue = vec![
            GraphStoreMaintenanceQueueItemProjection {
                vertex_ref: gleaph_graph_kernel::NodeId::from(7u8).into(),
                anchor_ordinal: 3,
                window_start_ordinal: 2,
                window_end_ordinal_exclusive: 5,
    ...
    fn formatter_formats_maintenance_queue_storage() {
        let storage = GraphStoreMaintenanceQueueStorageProjection {
            logical_len_bytes: 136,
            queue_len: 2,
            format_version: Some(1),
    ...
    fn formatter_formats_history_sequence() {
        let history = vec![
            GraphStoreWriteEventProjection::Edge(GraphStoreEdgeWriteProjection {
                operation: GraphStoreEdgeWriteOperation::Delete,
                path: GraphMutationPath::Base,
                refreshed: GraphStoreRefreshedVertices::new(vec![0], vec![]),
            }),
            GraphStoreWriteEventProjection::NodeDelete(GraphStoreNodeDeleteProjection {
                detached: true,
    ...
    fn formatter_formats_property_write_split_only() {
        let projection = GraphStoreWriteEventProjection::Property(GraphStorePropertyWriteProjection {
    ...
    fn formatter_formats_property_operations() {
        let history = vec![GraphStoreWriteEventProjection::Property(
    ...
    fn diagnostics_view_formats_facade_history() {
        ...
        assert_eq!(
            GraphStoreDiagnosticsView::formatted_write_history(&facade),
            vec!["bootstrap-vertices ordinals=1 refreshed=(0,0) fwd=[] rev=[]".to_owned()]
        );
        assert_eq!(
            GraphStoreDiagnosticsView::formatted_last_write_event(&facade),
            Some("bootstrap-vertices ordinals=1 refreshed=(0,0) fwd=[] rev=[]".to_owned())
        );
        assert_eq!(
            GraphStoreDiagnosticsView::debug_report(&facade),
