...
use crate::{GraphStoreMemorySlots, adjacency::GraphAdjacencyMemory};
...
pub type FixedSlotGraphPropertyStableMap<M> =
    StableBTreeMap<PropertyKey, StoredPropertyValue, GraphAdjacencyMemory<M>>;

pub fn open_fixed_slot_node_property_store<M: Memory + Clone>(
    slots: &GraphStoreMemorySlots<M>,
) -> FixedSlotGraphPropertyStableMap<M> {
    StableBTreeMap::init(slots.node_property_store())
}

pub fn open_fixed_slot_edge_property_store<M: Memory + Clone>(
    slots: &GraphStoreMemorySlots<M>,
) -> FixedSlotGraphPropertyStableMap<M> {
    StableBTreeMap::init(slots.edge_property_store())
}

pub fn snapshot_fixed_slot_graph_property_store<M: Memory + Clone>(
    map: &FixedSlotGraphPropertyStableMap<M>,
) -> BTreeMap<PropertyKey, StoredPropertyValue> {
    map.iter()
        .map(|entry| (entry.key().clone(), entry.value().clone()))
        .collect()
}

pub fn replace_fixed_slot_graph_property_store<M: Memory + Clone>(
    map: &mut FixedSlotGraphPropertyStableMap<M>,
...
pub(crate) fn btree_get_node_property<M: Memory>(
    map: &StableBTreeMap<PropertyKey, StoredPropertyValue, M>,
    node_id: NodeId,
    property_name: &str,
) -> Option<Value> {
...
mod tests {
    ...
    #[test]
    fn fixed_slot_property_store_round_trips() {
        let slots = GraphStoreMemorySlots::new(VecMemory::default());
        let mut node_store = open_fixed_slot_node_property_store(&slots);
        let mut edge_store = open_fixed_slot_edge_property_store(&slots);

        node_store.insert(
            PropertyKey::node(NodeId::from(1u8), "name"),
            StoredPropertyValue(Value::Text("alice".into())),
        );
        edge_store.insert(
            PropertyKey::edge(7, "weight"),
            StoredPropertyValue(Value::Int64(42)),
        );

        let reopened_nodes = open_fixed_slot_node_property_store(&slots);
        let reopened_edges = open_fixed_slot_edge_property_store(&slots);
        assert_eq!(
            snapshot_fixed_slot_graph_property_store(&reopened_nodes)
                .get(&PropertyKey::node(NodeId::from(1u8), "name")),
            Some(&StoredPropertyValue(Value::Text("alice".into())))
        );
        assert_eq!(
            snapshot_fixed_slot_graph_property_store(&reopened_edges)
                .get(&PropertyKey::edge(7, "weight")),
            Some(&StoredPropertyValue(Value::Int64(42)))
        );
    }
}
