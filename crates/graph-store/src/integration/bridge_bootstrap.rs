...
use crate::facade::{
    GraphStoreEdgeLogicalLocatorMapping, GraphStorePropertyIndexTouchedSections,
    GraphStorePropertyMutationWriteSummary, GraphStoreRefreshedVertices, GraphStoreStore,
    GraphStoreVertexOrdinalMapping,
};
use crate::low_level::{EdgeDirectedMetaPair, EdgeInsertPath, EdgeMeta, VertexRef};

use super::{
    GraphStoreKernelBootstrapBridge, GraphStoreOverlayBootstrapGraphSummary,
    GraphStoreOverlayEdgeBootstrapSummary, GraphStoreOverlayEdgeWriteSummary,
    GraphStoreOverlayInsertEdgeSummary, GraphStoreOverlayNodeBootstrapSummary,
    GraphStoreOverlayNodeDeleteSummary, GraphStoreOverlayWriteEvent, OVERLAY_SUMMARY_HISTORY_LIMIT,
...
impl<'a, S: GraphStoreStore> GraphStoreKernelBootstrapBridge<'a, S> {
    ...
    pub fn new(store: S, memory: &'a S::Mem) -> Self {
        ...
        if let Some((label_ids, next_label_id, index, gc_state)) =
            bridge.open_vertex_label_state()
        {
    ...
    pub fn vertex_ordinals(&self) -> &[GraphStoreVertexOrdinalMapping] {
    ...
    fn record_write_event(&mut self, event: GraphStoreOverlayWriteEvent) {
    ...
    pub(crate) fn record_property_write_summary(
        &mut self,
        summary: GraphStorePropertyMutationWriteSummary,
    ) {
        self.record_write_event(GraphStoreOverlayWriteEvent::Property(summary.clone()));
    ...
    pub(crate) fn patch_pending_property_summaries_after_stable_flush(
        &mut self,
        refreshed: GraphStoreRefreshedVertices,
    ) {
        for event in &mut self.write_history {
            if let GraphStoreOverlayWriteEvent::Property(summary) = event
                && summary.is_pending_stable_flush()
            {
                ...
                {
                    summary.flushed_sections = GraphStorePropertyIndexTouchedSections {
                        property_store: true,
                        logical_index: true,
                        node_store: true,
                    };
    ...
    pub(crate) fn record_insert_edge_summary(&mut self, summary: GraphStoreOverlayInsertEdgeSummary) {
        self.record_write_event(GraphStoreOverlayWriteEvent::InsertEdge(summary.clone()));
    ...
    pub(crate) fn record_edge_write_summary(&mut self, summary: GraphStoreOverlayEdgeWriteSummary) {
        self.record_write_event(GraphStoreOverlayWriteEvent::Edge(summary.clone()));
    ...
    pub(crate) fn record_node_delete_summary(&mut self, summary: GraphStoreOverlayNodeDeleteSummary) {
        self.record_write_event(GraphStoreOverlayWriteEvent::NodeDelete(summary.clone()));
    ...
    pub(crate) fn record_node_bootstrap_summary(
        &mut self,
        summary: GraphStoreOverlayNodeBootstrapSummary,
    ) {
        self.record_write_event(GraphStoreOverlayWriteEvent::BootstrapNode(summary));
    }

    pub(crate) fn record_edge_bootstrap_summary(
        &mut self,
        summary: GraphStoreOverlayEdgeBootstrapSummary,
    ) {
        self.record_write_event(GraphStoreOverlayWriteEvent::BootstrapEdge(summary));
    }

    pub(crate) fn record_bootstrap_graph_summary(
        &mut self,
        summary: GraphStoreOverlayBootstrapGraphSummary,
    ) {
        self.record_write_event(GraphStoreOverlayWriteEvent::BootstrapGraph(summary));
    }
    ...
    pub fn bootstrap_node(
        ...
        self.record_node_bootstrap_summary(GraphStoreOverlayNodeBootstrapSummary {
            node: record.clone(),
    ...
    pub fn bootstrap_edge_with_shard_canister_dst(
        ...
        self.persist_shard_canister_directory();
    ...
    fn insert_edge_record_with_meta(
        ...
        let refreshed = GraphStoreRefreshedVertices::new(
        ...
        if bootstrap_event {
            self.record_edge_bootstrap_summary(GraphStoreOverlayEdgeBootstrapSummary {
                edge: record.clone(),
                path,
                refreshed,
            });
            ...
            let (total_displacement, max_displacement) = summary
                ...
                .unwrap_or((0, 0));
            self.record_insert_edge_summary(GraphStoreOverlayInsertEdgeSummary {
