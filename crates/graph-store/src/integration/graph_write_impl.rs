...
use crate::facade::GraphStorePropertyMutationWriteSummary;
use crate::low_level::{EdgeDirectedMetaPair, GraphMutationPath};

use super::{
    GraphStoreKernelOverlayGraph, GraphStoreOverlayEdgeMutationKind, GraphStoreOverlayEdgeWriteSummary,
    GraphStoreOverlayNodeDeleteSummary,
};

impl<'a, S: super::GraphStoreStore> GraphStoreKernelOverlayGraph<'a, S> {
    fn delete_edge_without_flush(&mut self, edge_id: EdgeId) -> GraphResult<()> {
        ...
        self.bridge
            .record_edge_write_summary(GraphStoreOverlayEdgeWriteSummary {
                operation: GraphStoreOverlayEdgeMutationKind::Delete,
                path: path.mutation,
...
impl<'a, S: super::GraphStoreStore> GraphWrite for GraphStoreKernelOverlayGraph<'a, S> {
    fn insert_node(
        &mut self,
        labels: &[String],
    ...
    fn set_edge_label(&mut self, edge_id: EdgeId, label: Option<&str>) -> GraphResult<EdgeRecord> {
        ...
        self.bridge
            ...
            .map(|summary| {
                self.bridge
                    .record_edge_write_summary(GraphStoreOverlayEdgeWriteSummary {
                        operation: GraphStoreOverlayEdgeMutationKind::ReplaceLabel,
    ...
    fn delete_node(&mut self, node_id: NodeId, detach: bool) -> GraphResult<()> {
        ...
        if let Some(mapping) = self.bridge.vertex_ordinal_by_node_id.get(&node_id).copied() {
            ...
            for label in &removed_node.labels {
                if let Some(label_id) = self.bridge.lookup_label_id(label) {
                    ...
                    let mut state_store = self.bridge.open_vertex_label_state_store();
                    if let Some(membership) = self.bridge.vertex_label_index.by_label.get(&label_id)
                    {
        ...
        self.bridge
            .record_node_delete_summary(GraphStoreOverlayNodeDeleteSummary {
                detached: detach,
    ...
    fn flush(&mut self) -> GraphResult<()> {
        ...
        self.bridge
            .patch_pending_property_summaries_after_stable_flush(
                crate::facade::GraphStoreRefreshedVertices::new(fwd, rev),
            );
        Ok(())
    }
}
