...
impl<'a, M: Memory> GraphStoreBatchSession<'a, M> {
...
impl<'a, S: GraphStoreStore> GraphStoreStoreAdapter<'a, S> {
    ...
    pub fn flush_dirty(&mut self) -> GraphStoreResult<GraphStoreRefreshedVertices> {
        let (forward, reverse) = self
            .store
            .try_refresh_and_write_dirty_to_stable_memory(self.memory)?;
        Ok(GraphStoreRefreshedVertices::new(forward, reverse))
    }
...
impl<'a, S: GraphStoreStore> GraphStoreService for GraphStoreStoreAdapter<'a, S> {
