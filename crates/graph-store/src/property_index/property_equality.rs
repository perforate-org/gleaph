...
pub type FixedSlotPropertyEqualityMap<M> =
    StableBTreeMap<PropertyIndexKey, PropertyIndexEntry, GraphAdjacencyMemory<M>>;

pub fn open_fixed_slot_property_equality_map<M: StableMemoryTrait + Clone>(
    slots: &GraphStoreMemorySlots<M>,
) -> FixedSlotPropertyEqualityMap<M> {
    StableBTreeMap::init(slots.property_index())
}

pub fn snapshot_fixed_slot_property_equality_map<M: StableMemoryTrait + Clone>(
    map: &FixedSlotPropertyEqualityMap<M>,
) -> PropertyEqualityStableMap {
    let mut snapshot = empty_property_equality_map();
...
pub fn replace_fixed_slot_property_equality_map<M: StableMemoryTrait + Clone>(
    map: &mut FixedSlotPropertyEqualityMap<M>,
    entries: &PropertyEqualityStableMap,
) {
...
mod tests {
    ...
    #[test]
    fn fixed_slot_property_equality_map_round_trips() {
        let slots = GraphStoreMemorySlots::new(VectorMemory::default());
        let mut map = open_fixed_slot_property_equality_map(&slots);
        map.insert(
            PropertyIndexKey::node(NodeId::from(7u8), "uid", vec![1, 2, 3]),
            PropertyIndexEntry::empty(),
        );

        let reopened = open_fixed_slot_property_equality_map(&slots);
        let snapshot = snapshot_fixed_slot_property_equality_map(&reopened);
        assert_eq!(snapshot.len(), 1);
        assert!(snapshot.iter().any(|entry| matches!(
            entry.key(),
            PropertyIndexKey {
