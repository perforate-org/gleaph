...
//!
//! Complements `docs/graph-store-target-design.md` (Internet Computer: full property search stack).
//!
...
//! `RUSTFLAGS='-C target-feature=+simd128' cargo bench -p gleaph-graph-store --bench property_search_hot_path`.
...
fn setup_flushed_blob_prop_index(
    entry_count: usize,
    value_len: usize,
) -> (GraphStoreVecMemory, GraphStore) {
    let memory = GraphStoreVecMemory::default();
...
fn setup_flushed_uid_index(entry_count: usize) -> (GraphStoreVecMemory, GraphStore) {
    let memory = GraphStoreVecMemory::default();
    let mut facade = GraphStore::bootstrap_empty(memory.clone()).expect("bootstrap");
    for i in 0..entry_count {
        let id = NodeId::try_from((i + 1) as u64).expect("NodeId");
