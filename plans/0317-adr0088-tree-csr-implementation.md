...
todos:
  ...
  - id: "tree-mode-read-dispatch"
    content: "Implement the mode dispatch in the bucket access constructor: `out_edges_iter`, `out_edges_collect`, `visit_edges`, `prefix_scan`, and `random_ordinal_access` (production paths that today always go through `EdgeStore`). When the bucket's tree-mode bit is set, the dispatch routes to a new `tree_mode_out_edges_iter` (using `LtbRawBlockStore::for_each_chunk` for the chunk-buffer pattern). When the bit is clear, the existing slab path runs unchanged. The dispatch lives in one constructor — reviewers must not see a mode branch in rope / PMA / placement code."
    status: pending
  - id: "tree-mode-edge-write-dispatch"
    content: "Implement `insert_edge` (and `remove_edge_at_slot`, `direct_unlink_log_*`) for the tree-mode path: when the bucket is in tree mode, the new edge is appended into the LTB blocks (single `write_payload_partial` at the tail offset if the tail block has room; otherwise mint a new tail block and append at `tail_first_slot`). Tombstone semantics stay above the LTB store layer; this method only mutates the canonical row."
    status: pending
  - id: "deepen-flatten"
    content: "Implement `deepen()` and `flatten()` for the tree-mode descriptor. `deepen` runs when the derived root length would exceed `R_max = 1024`: reserve interior blocks, copy current root ids into the interior blocks, rewrite the span to the interior ids (right-spine partial allowed), and publish. `flatten` is the inverse, only ever inside compaction/maintenance. Both are level-generic (2→3 identical one level up). Fail-closed at `MAX_DEPTH = 3` per ADR 0088 §4."
    status: pending
  - id: "wasm-budget-recheck"
    content: "Run `cargo build --release --target wasm32-unknown-unknown --features canbench` from `crates/ic-stable-lara/`. The exported-name budget must still be under the 20K PocketIC limit. Plan 0317 adds no new canbench benches (it only changes production code paths); expected: ~16,776 chars / 3,224 chars headroom preserved."
    status: pending
  - id: "gate-2-recheck-on-virtual-memory"
    content: "Re-run Gate 2 canbench benches against the production `VirtualMemory<DefaultMemoryImpl>` backend: `tcsr_4096_full_scan_descending`, `tcsr_65536_full_scan_descending`, `tcsr_4096_random_ordinal_access`, `tcsr_4096_insert_grow`, etc. Record ins/edge for each row and confirm production backend is within ±20% of the `VectorMemory` test numbers from Plan 0322 (~41 ins/edge at 4K scan). If production numbers regress beyond ±20%, open a follow-up slice to investigate the regression."
    status: pending
  - id: "plan-validator"
    content: "Run `python3 ~/.agents/skills/plan/scripts/validate_plan.py plans/0317-adr0088-tree-csr-implementation.md --phase final` and confirm structurally-valid final-phase verdict before reporting completion."
    status: pending
isProject: false
---

# ADR 0088 Tree-CSR implementation in LabeledLaraGraph

## Objective

Wire the Plan 0322-complete Tree-CSR prototype (`tree_csr_prototype.rs` + `ltb_raw_block_store.rs` + `tree_csr_high_degree_test.rs`) into the production `LabeledLaraGraph` as the **third bucket backing state** alongside bypass and slab. The goal is a single-commit, atomic transition: every bucket still resolves to one of three states; the tree-mode path uses `LtbRawBlockStore::for_each_chunk` for the chunk-buffer iter pattern, `read_payload_partial` for point lookups, and `write_payload_partial` for sub-block mutations. The mode dispatch lives in **one constructor** (the bucket access point) — reviewers must not see a mode branch in rope / PMA / placement code per [ADR 0088 §Encapsulation](../design/adr/0088-tree-csr-mode-for-high-degree-label-buckets.md).

Success signal:

- `LabelBucket` packed word gains a tree-mode flag bit (bit 63); `LabelBucket::BYTES == 29` is preserved; reserved-bit validator is updated to ignore bit 63 while keeping bits 60–62 reserved.
- `LabeledLaraGraph<E, M>` has a new `ltb: LtbRawBlockStore<M>` field; `new(...)` and `init(...)` accept one extra `M: Memory` argument. `BidirectionalLabeledLaraGraph` accepts two extra (forward + reverse).
- `promote_bypass_to_tree_mode(vid, label)` follows the failure-atomic reserve/commit/publish template (mirrors `promote_bypass_to_bucket_mode`).
- The caps `T_PROMOTE` and `R_MAX` are enforced on `alloc_space = stored_slots + alloc_gap`, not on `stored_slots` alone. For CSR slab mode `alloc_space ≤ T_PROMOTE = 4096 slots (16 KiB)`; for tree mode `alloc_space = stored_slots ≤ R_MAX = 1024 slots (4 KiB)` (gap-0 invariant). LARA rebalance naturally keeps `alloc_space` bounded by `T_PROMOTE` via weighted gap allocation. See §Step 3.5 below for the full cascade (bucket cap → vertex cap of `MAX_BUCKETS_PER_VERTEX = 1024`).
- `out_edges_iter`, `out_edges_collect`, `visit_edges`, `prefix_scan`, `random_ordinal_access`, `insert_edge`, `remove_edge_at_slot` dispatch on the tree-mode flag bit and route to the LTB-backed path when set.
- `deepen()` and `flatten()` work level-generically and fail-closed at `MAX_DEPTH = 3`.
- Gate 2 canbench benches re-run on the production `VirtualMemory` backend; numbers within ±20% of the Plan 0322 `VectorMemory` test numbers.
