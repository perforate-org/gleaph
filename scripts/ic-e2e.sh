...
# Usage: ic-e2e.sh <project-root> [target | --inline] [--close]
#
# target:
#   (none)      - run the full E2E suite (default)
#   --all       - alias for the full E2E suite
#   smoke       - run only the smoke test
#   --inline    - run the smoke test directly in the current terminal
...
if [ "$INLINE" -eq 1 ]; then
    exec env -u POCKET_IC_BIN cargo test -p gleaph-pocket-ic-tests --test smoke -- --nocapture
fi
...
TMP=$(mktemp /tmp/gleaph_ic_e2e.XXXXXX.scpt)
