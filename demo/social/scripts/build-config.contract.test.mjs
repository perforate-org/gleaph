...
import {
  buildSocialLoadArtifact,
  normalizeCreatedAt,
} from "./social-load-artifact.mjs";

const buildConfigPath = fileURLToPath(
  new URL("./build-config.mjs", import.meta.url),
);
...
test("typed artifact preserves vertex and edge order and source-id closure", () => {
  ...
  const artifact = buildSocialLoadArtifact({
    graph,
    demoId: (id) => ids.get(id),
  });
  assert.equal(artifact.format_version, 1);
  assert.deepEqual(
    artifact.vertices.map(({ source_id }) => source_id),
    ["u", "p"],
  );
  assert.deepEqual(
    artifact.edges.map(({ source, target }) => [source, target]),
    [["u", "p"]],
  );
  assert.throws(
    () =>
...
test("full generator emits byte-identical typed load artifacts in isolated roots", () => {
  ...
  for (const artifact of ["vertices.jsonl", "edges.jsonl"]) {
    assert.deepEqual(
      readFileSync(join(outputs[0], "seeds", artifact)),
      readFileSync(join(outputs[1], "seeds", artifact)),
    );
  }
});

const readJsonlRows = (path) =>
...
test("default generator preserves the tracked 5x/20x workload and endpoint closure", () => {
  ...
  const vertices = readJsonlRows(join(root, "seeds", "vertices.jsonl"));
  const edges = readJsonlRows(join(root, "seeds", "edges.jsonl"));
  assert.equal(graph.nodes.length, 7244);
  assert.equal(graph.edges.length, 47230);
  assert.equal(vertices.length, 7244);
  assert.equal(edges.length, 47230);

  const vertexIds = new Set(vertices.map(({ source_id }) => source_id));
  for (const edge of edges) {
    assert.ok(vertexIds.has(edge.source), `missing source ${edge.source}`);
    assert.ok(vertexIds.has(edge.target), `missing target ${edge.target}`);
  }
});
