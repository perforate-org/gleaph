...
const PREPARED_DIR = join(APP_ROOT, "prepared");
...
for (const post of posts) {
  ...
  const homeFeedRecipients = new Set([
    post.userId,
    ...(followsByTarget.get(post.userId) ?? []),
  ]);
...
for (const recipientId of new Set(
  homeFeedEntries.map((entry) => entry.recipientId),
)) {
  const recipientEntries = homeFeedEntries
    .filter((entry) => entry.recipientId === recipientId)
    .sort((a, b) => {
      const hashA = sha256Hex(
        `social-demo:home-feed-order:${recipientId}:${a.postId}`,
      );
      const hashB = sha256Hex(
        `social-demo:home-feed-order:${recipientId}:${b.postId}`,
      );
      return hashA.localeCompare(hashB);
    });
  ...
  for (let index = 1; index < recipientEntries.length; index += 1) {
    if (recipientEntries[index - 1].userId !== recipientEntries[index].userId)
      continue;
    const replacement = recipientEntries.findIndex(
      (entry, candidate) =>
        candidate > index && entry.userId !== recipientEntries[index].userId,
    );
    if (replacement >= 0) {
...
// ---------------------------------------------------------------------------
const socialLoad = buildSocialLoadArtifact({
  graph,
  demoId,
...
for (const scenarioFile of sortedYamlFiles(scenariosDir)) {
  ...
  // native format; `gleaph prepared apply` registers it). Strip leading `///` doc-comment lines
  // so the frontend copy holds the query text only.
  const preparedPath = join(PREPARED_DIR, `${doc.preparedQueryId}.gql`);
  if (!existsSync(preparedPath)) {
    throw new Error(
      `scenario ${doc.id} has no prepared source at ${preparedPath}`,
    );
  }
  const preparedText = readFileSync(preparedPath, "utf8");
  doc.preparedQuery = preparedText
    .split(/\r?\n/)
    .filter((line) => !line.trimStart().startsWith("///"))
    .join("\n")
    .trim();
  if (!doc.preparedQuery) {
    throw new Error(
      `scenario ${doc.id} prepared source ${preparedPath} is empty`,
    );
  }
  scenarios.push(doc);
}
...
const buildTsScenarios = () => {
  ...
  return `// Generated by demo/social/scripts/build-config.mjs.
// Do not edit manually; change the YAML files under config/ and the prepared/
// sources, then rerun the build.
...
const buildTsUserAvatars = () => {
  const entries = users
    .map(
      (user) =>
        `  ${JSON.stringify(user.name)}: ${JSON.stringify(`/avatars/${user.id}.svg`)}`,
    )
    .join(",\n");
  return `// Generated by demo/social/scripts/build-config.mjs.
...
// social-graph.json is the canonical manifest (pretty-printed; consumed by the
// contract tests and the future vector work, not by the CLI flow).
writeFileSync(
  join(SEEDS_DIR, "social-graph.json"),
  `${JSON.stringify(graph, null, 2)}\n`,
);

// vertices.jsonl / edges.jsonl are the `gleaph load` artifact: one typed row per
// line (NDJSON), which the CLI validates and chunks as a bounded-memory stream
// (the single-file JSON family is capped at 64 MiB and recommends NDJSON for
// larger data). Each line is the same row shape the single document used.
const verticesJsonl =
  socialLoad.vertices.map((vertex) => JSON.stringify(vertex)).join("\n") + "\n";
const edgesJsonl =
  socialLoad.edges.map((edge) => JSON.stringify(edge)).join("\n") + "\n";
writeFileSync(join(SEEDS_DIR, "vertices.jsonl"), verticesJsonl);
writeFileSync(join(SEEDS_DIR, "edges.jsonl"), edgesJsonl);
...
if (
  socialLoad.vertices.length !== graph.nodes.length ||
  socialLoad.edges.length !== graph.edges.length
) {
  throw new Error(
    "Typed social-load artifact does not preserve graph source order",
  );
}

console.log(
  `Wrote 6 artifacts: social-graph.json, vertices.jsonl, edges.jsonl, scenarios.generated.ts, scenarios.generated.json, userAvatars.generated.ts`,
);
