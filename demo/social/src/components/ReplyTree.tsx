...
export function ReplyTree(props: { rows: PostRow[]; formatDate: (seconds: bigint) => string }) {
      ...
      <For each={buildReplyForest(props.rows)}>
        {(node) => (
          <ReplyBranch
            ...
            showParentPreview={
              node.row.parentPostId !== undefined && !visiblePostIds.has(node.row.parentPostId)
            }
          />
        )}
      </For>
          ...
          <For each={props.node.replies}>
            {(reply) => (
              <div class="mb-3 last:mb-0">
                <ReplyBranch node={reply} formatDate={props.formatDate} />
              </div>
            )}
          </For>
        </div>
