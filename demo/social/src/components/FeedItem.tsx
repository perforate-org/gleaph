...
export function FeedItem(props: FeedItemProps) {
  const { row, formatDate, showParentPreview = false } = props;
  const { t } = useI18n();
            ...
            <div class="mt-3 rounded-lg bg-slate-50 p-3">
              ...
              <ul class="mt-2 space-y-1 text-sm text-slate-700">
                ...
                <li>
                  ...
                  {t("feed.edgeToTopic")}{" "}
                  <span class="font-medium">{displayPostId(row.topicId)}</span>
                </li>
              </ul>
              <p class="mt-2 text-xs text-slate-500">{t("feed.seedLabelsNote")}</p>
            </div>
          )}
        </div>
      </div>
