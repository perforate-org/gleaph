  ...
  return (
    <figure class="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3">
      ...
      <div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-600" aria-hidden="true">
        <span>
          <span class="mr-1 inline-block h-2 w-2 rounded-full bg-rose-600" />
          {t("explanation.topicPathChartMysql")}
        </span>
        <span>
          <span class="mr-1 inline-block h-2 w-2 rounded-full bg-indigo-600" />
          {t("explanation.topicPathChartNeo4j")}
        </span>
      </div>
      <p class="mt-2 text-xs leading-relaxed text-slate-500">
        {t("explanation.topicPathChartCaption")}
      </p>
    </figure>
  );
}
