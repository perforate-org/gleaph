...
export const SCENARIO_DEFINITIONS: Record<ScenarioId, ScenarioDefinition> = {
  PublicTimeline: {
    ...
    graphSummary: "The graph materializes `IN_PUBLIC_FEED` edges from every public Post to a dedicated Feed vertex at seed time. Gleaph's labeled-edge insertion order becomes the chronological key, so a single fixed-label expansion with `LIMIT` returns newest-first without an explicit `ORDER BY`. The value is not speed; it is that the same vertex model will later power relationship-aware feeds without a new schema.",
    preparedQuery: "MATCH (feed:Feed {demo_id: 16})<-[:IN_PUBLIC_FEED]-(p:Post) RETURN p.demo_id AS post_id, p.body AS body, p.created_at AS created_at LIMIT 20",
    semanticVector: null,
  ...
  AliceHomeFeed: {
    ...
    graphSummary: "Gleaph materializes `IN_HOME_FEED` edges from every public Post to each follower User at seed time, so Alice's feed is a single bounded expansion from her user vertex. The feed is the natural result of the relationship shape, not an application-side assembly, and the default descending labeled-edge scan returns newest posts first with no runtime sort key.",
    preparedQuery: "MATCH (u:User {demo_id: 1})<-[:IN_HOME_FEED]-(p:Post) WHERE p.is_public = TRUE RETURN p.demo_id AS post_id, p.body AS body, p.created_at AS created_at LIMIT 20",
    semanticVector: null,
