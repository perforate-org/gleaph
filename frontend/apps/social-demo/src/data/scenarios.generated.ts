...
export const SCENARIO_DEFINITIONS: Record<ScenarioId, ScenarioDefinition> = {
  PublicTimeline: {
    ...
    preparedQuery: "MATCH (feed:Feed {demo_id: 16})<-[:IN_PUBLIC_FEED]-(p:Post)<-[:POSTED]-(author:User) RETURN p.demo_id AS post_id, author.name AS author_name, p.body AS body, p.created_at AS created_at ORDER BY p.order_index DESC LIMIT 20",
    semanticVector: null,
  ...
  AliceHomeFeed: {
    ...
    preparedQuery: "MATCH (u:User {demo_id: 1})<-[:IN_HOME_FEED]-(p:Post)<-[:POSTED]-(author:User) WHERE p.is_public = TRUE RETURN p.demo_id AS post_id, author.name AS author_name, p.body AS body, p.created_at AS created_at ORDER BY p.order_index DESC LIMIT 20",
    semanticVector: null,
  ...
  TopicPath: {
    ...
    preparedQuery: "MATCH (p:Post)-[has_topic:HAS_TOPIC]->(t:Topic) WHERE t.demo_id = 7 MATCH (u:User)-[follows:FOLLOWS]->(author:User)-[posted:POSTED]->(p) WHERE u.demo_id = 1 RETURN p.demo_id AS post_id, author.name AS author_name, follows.demo_edge_id AS follows_edge_id, posted.demo_edge_id AS posted_edge_id, t.demo_id AS topic_id, has_topic.demo_edge_id AS topic_edge_id, p.body AS body, p.created_at AS created_at ORDER BY p.order_index DESC",
    semanticVector: null,
