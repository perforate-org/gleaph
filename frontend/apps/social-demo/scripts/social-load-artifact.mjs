...
export function normalizeCreatedAt(payload) {
  ...
  if (typeof payload !== "string" && typeof payload !== "number" && typeof payload !== "bigint") {
    throw new Error("created_at must be a string or integer");
  }
  const text = String(payload);
  if (!/^[0-9]{12}$/.test(text)) {
    throw new Error("created_at must contain exactly 12 ASCII digits (YYYYMMDDHHmm)");
  }
  ...
  if (year < 1 || year > 9999) throw new Error("created_at year must be in 0001..=9999");
  ...
  const seconds =
    daysFromCivil(year, month, day) * 86_400n + BigInt(hour * 60 + minute) * 60n;
...
export function buildSocialLoadArtifact({ graph, demoId, demoGraph = "social" }) {
  ...
  if (sourceIds.size !== vertices.length) throw new Error("social-load source_id values must be unique");
  ...
  for (const node of graph.nodes) {
    if (node.embedding) {
      if (node.kind !== "post") throw new Error(`Non-Post node ${node.id} has embedding`);
  ...
  return { vertices, edges, embeddings };
}
