# graph-pma Low-level Spec

## Purpose

This document is the implementation-facing companion to
[`graph-pma-target-design.md`](/Users/yota/dev/gleaph-project/docs/graph-pma-target-design.md).
It exists to drive the rewrite of `crates/graph-pma` from low-level storage
...
The first rewritten `graph-pma` milestone should provide:
...
`graph-pma` should not treat adjacency as the only growable structure in stable
memory.
