# graph-store Target Design

## Status

- Target architecture document
- Written to clarify the intended end state of `crates/graph-store`
- The current implementation does **not** yet satisfy this document in full

## Why This Exists

The current `graph-store` implementation already establishes several useful
concepts:
...
- Flushed with the rest of the graph via `GraphStore::try_write_all_to_stable_memory`
...
2. **`GraphStore`** hydrates from those regions and exposes mutation +
   `try_write_all_to_stable_memory` (or incremental write paths) after updates.
3. **`GraphStoreKernelOverlayGraph`** implements **`GraphRead` / `GraphWrite`** for
...
- **Single kernel trait boundary:** all search and mutation semantics for the
  ...
  `G: GraphRead + GraphWrite` (backed by `graph-store`), not ad-hoc index calls
  in the executor.
...
- **Contract tests:** planner + executor + `GraphStoreKernelOverlayGraph` (or
...
- Pushing property-index logic into the executor or planner beyond **plan
  selection**—storage stays in `graph-store`.
