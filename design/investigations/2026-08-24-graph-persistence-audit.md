...
| F2 | Edge row payload is 30-bit (~1.07e9 vertices/shard) while `VertexId` is u32 and slab indices are 36-bit — undocumented shard-capacity ceiling | A/D | **P2** |
...
| F8 | LARA header reserved-byte policy is inconsistent (some zeroed+checked, some declared but not written) | D3' hygiene | P3 |
| F9 | Dead code: `PropertyCatalog` type alias never instantiated (`facade/stable/property_catalog.rs`) | G/cleanup | P3 |
...
| F11 | Mixed endianness across facade keys (BE for ordered map keys 32–34/52; LE for `EffectKey`, `PhysicalIndexId`, manual codecs) | B hygiene | P4 |
...
| 51 | canonical export scopes | **explicitly no envelope**, documented regeneration contract (`graph-kernel/src/canonical_export.rs:190-193`) | C | ⚠ needs production-gate decision |
...
| C2 | Document the 30-bit edge-payload ceiling as an explicit shard-capacity bound (~1.07e9 local vertex refs) in `design/storage/lara.md` / dgap contract, and either bless it as final or schedule widening | **P2** | closes F2; pure documentation + decision |
