...
|--------|--------|------------|
| `Expand` | Partial | `{min,max}` hop-count var-length; `label_expr` (`-/A\|B/->`, `-[e:A\|B]->`); index/payload/vector fusion per named label in `label_expr`; `GLEAPH.WEIGHT` decode per bound edge label |
