...
| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-20 | Graph MemoryId 52 `INDEX_PENDING_FLOOR` now projects one fixed 17-byte key for each tracked ordinary source row in the first-delivery outbox (46) and repair journal (41). Appends, acknowledgements, removals, and quarantine transitions co-update the durable owner and exact floor synchronously. The first key gives `index_pending_min_mutation_id`; no raw queue scan, timer state, rebuild path, or alternate public floor remains. |
...
| 2026-06-21 | ADR 0029 Phase 2 sync: durable repair-journal rows (D5, region 41) carry their originating federated `mutation_id` (`0` = untracked), making graph-index posting lag observable per mutation through the Graph-owned `index_pending_min_mutation_id` boundary.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 2026-06-20 | Phase 4 completed (P4): the compensation-failure trap is retired across all three flush paths (`index/pending.rs`, `edge_pending.rs`, `label_pending.rs`) — the branch now journals the full batch and returns an error instead of `ic_cdk::trap`, since idempotent journal re-application converges the index to the store regardless of partial compensation state (store becomes the source of truth; no whole-message rollback). Refinement: the durable `index-dirty` marker + the catalog-driven _scan_ rebuild from the original D5 wording are re-sequenced into phase 5 (they need D6's index-side posting-purge primitive and share its machinery); the journal alone closes P4. Phases 5–6 pending.                                                                                                                                                                                                                                                 |
...
| 2026-08-22 | D6 durability amendment (GAP-2026-08-20-005 resolved): the remaining-reference gate is removed as unsound — posting scopes are disjoint per physical namespace, so every dropped definition retires its own namespace **unconditionally**. The Router co-writes the catalog removal with a durable retirement record keyed by `PhysicalIndexId` (`ROUTER_INDEX_RETIRED`, MemoryId 54; see [ADR 0007](0007-stable-memory-layout.md)) freezing drain targets before the destructive mutation and persisting one resume cursor per pending target. Inline bounded rounds cover small purges inside the DDL; holds defer to a recovery-timer lane until the last frozen target confirms `done`. `DROP INDEX` succeeds after the co-write and never fails on purge transport errors. Owning regressions: `dropping_one_of_two_indexes_on_shared_property_does_not_leak_its_namespace` plus router unit tests in `router/src/index_retirement.rs`. |
| 2026-08-03 | [ADR 0059](0059-create-index-migration-backfill.md) accepted as a partially implemented amendment for migration-driven online index backfill (production Router driver implemented; PocketIC E2E/upgrade validation pending). The Graph-owned export, generation, and Active-only activation rules are additional lifecycle policy; this ADR's router-sourced catalog and durable repair-journal consistency rules remain authoritative.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| P7  | High     | `DROP INDEX` leaves orphan postings on graph-index — **fixed (D6, phase 5; durable retirement 2026-08-22)**                                                                     | `router::drop_index` co-writes the catalog removal with a durable per-`PhysicalIndexId` retirement record and fans the bounded `admin_purge_property_postings` primitive out across the graph's index canisters unconditionally (scopes are disjoint per namespace); the recovery timer drains any holds                                                                                                                                    |
...
_scan_ rebuild (folded in from D5/phase 4) are **not** implemented. The durable
...
- **Compensation failure (P4):** **do not trap.** _(Implemented, refined.)_ The
...
4. **(implemented)** Durable repair journal
   ...
   **Refinement vs the original D5 wording:** D5 proposed an `index-dirty` marker
   - a separate catalog-driven _scan_ rebuild as the compensation-failure
