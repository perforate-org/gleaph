...
1. **Identity.** The Router interns each declared vertex nested leaf path (for example
   ...
   `IndexedVertexMembership` gains an optional `field_path` with the same meaning and empty
   default as the edge membership field, keeping one catalog shape for both domains.
...
1. Kernel + DDL: add `field_path` to `IndexedVertexMembership` and the registration args;
   Router DDL interns leaf identities and emits memberships.
2. Graph mutation dispatch: leaf-aware resolution in the vertex change path, sharing the edge
   resolver; rejection-free absence semantics covered by tests.
3. Backfill: extend the vertex export to walk records along declared paths under the same
   cursor protocol as the edge export's domains (ADR 0059 one-opaque-export rule).
4. Planner: seed anchors and range/equality candidates against interned leaf identities;
   statistics reuse the existing per-property estimates unchanged.
...
- The kernel wire type changes additively (`IndexedVertexMembership` and registration args gain
  an optional field); pre-production requires fresh state when the catalog layout rolls forward,
  per the repository's pre-production simplicity rule.
