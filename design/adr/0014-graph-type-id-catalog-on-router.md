...
Anchor timestamp: 2026-06-13 13:17:41 UTC +0000
...
- [ADR 0013](0013-gql-graph-type-catalog-on-router.md) — `GraphCatalog` on regions **30–31**;
...
|--------|--------|-----|-------|
| 32 | `ROUTER_GRAPH_TYPE_BY_NAME` | [`object_name_key`] → **`GraphTypeId`** | catalog |
| 33 | `ROUTER_GRAPH_TYPE_BY_ID` | **`GraphTypeId`** → name string | catalog |

Thread-local: **`ROUTER_GRAPH_TYPE_CATALOG`** (`BidirectionalCatalog<GraphTypeId, …>`), initialized
from regions 32–33 (mirror [`ROUTER_GRAPH_CATALOG`] at 24–25).

**Stable repack:** router grows **32 → 34** regions (ADR 0007 gate). Dev snapshot discard acceptable
pre-production.

Existing regions **30–31** stay the [`GraphCatalog`] payload maps; only **key types and wire
variants** change (§3).
...
|-----|--------|------------|---------|
| `type_map` | 30 | **`GraphTypeId`** | [`StorableGraphTypeDefinition::V1`] (unchanged value codec) |
| `binding_map` | 31 | **`GraphId`** | [`GraphSchemaBinding::V2`] |
...
| Layer | Owns |
|-------|------|
| **`gleaph-graph-kernel`** | `GraphTypeId`, `CatalogId` impl, stable layout symbols 32–33 |
...
| Phase | Scope | Status |
|-------|--------|--------|
| **T0** | `GraphTypeId` in `graph-kernel`; `CatalogId`; layout registry regions 32–33 | Planned |
| **T1** | Router `ROUTER_GRAPH_TYPE_CATALOG` + init; `GraphTypeLookup` in catalog crate | Planned |
| **T2** | `type_map` keys → `GraphTypeId`; `GraphSchemaBinding::V2`; update unit tests + canbench | Planned |
| **T3** | Upgrade migration V1→V2 (or dev discard); update `ROUTER_STABLE_LAYOUT` count **34** | Planned |
| **T4** | PocketIC regression: `CREATE GRAPH TYPE` + `TYPED` + drop cascade | Planned |
...
2. **If retaining snapshots:** one-shot upgrade hook:
   ...
   - Clear legacy string-keyed btree or repack region 30
...
|-------|----------|----------|
| Mount `GraphCatalog` on router | ✓ regions 30–31 | unchanged regions; **key / wire change** |
...
| [storage/stable-memory-inventory.md](../storage/stable-memory-inventory.md) | Planned regions 32–33; router count 34 | **This patch** |
| [gql/layers.md](../gql/layers.md) | Note graph type name intern at catalog DDL | Pending T1 |
