...
| 2026-06-13 | Accepted: T0–T4 — `GraphTypeId`, regions 24–25, `GraphTypeLookup`, V2 bindings, PocketIC regression. |
...
- [ADR 0013](0013-gql-graph-type-catalog-on-router.md) — `GraphCatalog` on regions **22–23**;
...
|--------|--------|-----|-------|
| 24 | `ROUTER_GRAPH_TYPE_BY_NAME` | [`object_name_key`] → **`GraphTypeId`** | catalog |
| 25 | `ROUTER_GRAPH_TYPE_BY_ID` | **`GraphTypeId`** → name string | catalog |

Thread-local: **`ROUTER_GRAPH_TYPE_CATALOG`** (`BidirectionalCatalog<GraphTypeId, …>`), initialized
from regions 24–25 (mirror [`ROUTER_GRAPH_CATALOG`] at 15–16).

**Stable repack:** router grows **32 → 34** regions (ADR 0007 gate). Dev snapshot discard acceptable
pre-production.

Existing regions **22–23** stay the [`GraphCatalog`] payload maps; only **key types and wire
variants** change (§3).
...
|-----|--------|------------|---------|
| `type_map` | 22 | **`GraphTypeId`** | [`StorableGraphTypeDefinition::V1`] (unchanged value codec) |
| `binding_map` | 23 | **`GraphId`** | [`GraphSchemaBinding::V2`] |
...
| Layer | Owns |
|-------|------|
| **`gleaph-graph-kernel`** | `GraphTypeId`, `CatalogId` impl, stable layout symbols 24–25 |
...
| Phase | Scope | Status |
|-------|--------|--------|
| **T0** | `GraphTypeId` in `graph-kernel`; `CatalogId`; layout registry regions 24–25 | **Implemented** |
...
| **T3** | Upgrade migration V1→V2 (or dev discard); update `ROUTER_STABLE_LAYOUT` count **34** | **Implemented** (dev discard; legacy V1 `TypeRef(String)` → `Unsupported` on read) |
...
2. **If retaining snapshots:** one-shot upgrade hook:
   ...
   - Clear legacy string-keyed btree or repack region 22
...
|-------|----------|----------|
| Mount `GraphCatalog` on router | ✓ regions 22–23 | unchanged regions; **key / wire change** |
...
| [storage/stable-memory-inventory.md](../storage/stable-memory-inventory.md) | Regions 24–25; router count 34 (0–33) | **Implemented** |
