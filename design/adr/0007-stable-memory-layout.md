...
| Graph-vector-index  | 13           | 0–14     | Router auth, shard catalog, ownership config, index defs, centroid meta, reserved centroids, subject clock, partition heads, page meta + row slab (ADR 0064 §7 two-table format; MemoryIds 10/13), rebuild lifecycle state (ADR 0031 Slice 7), maintenance scan state (ADR 0031 Slice 10); MemoryIds 8/11 unallocated (retired id reverse maps)                                                                                           |
