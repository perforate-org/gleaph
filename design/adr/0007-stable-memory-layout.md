...
| 2026-08-11 | [ADR 0064](0064-vector-index-architecture.md) §5 adds `VECTOR_DELETED_SUBJECTS` (MemoryId 17), a maintenance GC list keyed by `(shard, tombstone stamp, subject)`. Together with `VECTOR_GC_CURSOR` (`Option<DeletedSubjectKey>`, MemoryId 16), it gives bounded GC a stable resume key while subject-map slots are removed. The vector canister now has **18 numbered regions (0–17; 16 allocated)**. |
| 2026-08-08 | [ADR 0064](0064-vector-index-architecture.md) §5 adds the vector canister's per-shard watermark pair `VECTOR_SHARD_WATERMARKS` (MemoryId 15) and the subject-map GC resume cursor `VECTOR_GC_CURSOR` (MemoryId 16) bounding the subject map: `graph_watermark` (advanced by the graph shard's `vector_sync_batch`) and `router_watermark` (advanced by the Router's `vector_sync_batch`). A deleted subject-map entry with `stamp <= min(both)` for its shard is GC'd. Vector canister now has **17 regions (0–16)**.                                                                                 |
...
| Vector canister     | 16 allocated / 18 slots | 0–17 | Router auth, shard catalog, ownership config, index defs, centroid meta, reserved centroids, subject clock, partition heads, page meta + row slab (ADR 0064 §7 two-table format; MemoryIds 10/13), rebuild lifecycle state (ADR 0031 Slice 7), maintenance scan state, per-shard watermarks, `VECTOR_GC_CURSOR` (`Option<DeletedSubjectKey>`), and `VECTOR_DELETED_SUBJECTS` GC list (ADR 0064 §5); MemoryIds 8/11 unallocated (retired id reverse maps). IVF projections rebuild from active Vector rows; full durable-row loss requires client re-ingestion. |
