...
Anchor timestamp: 2026-08-21 09:03:02 UTC +0000

## Revision history

| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-21 | Router adds `ROUTER_VECTOR_INGEST_OUTBOX` (MemoryId 53), a canonical `StableBTreeMap<u64, VectorIngestOutboxState>` holding one immutable Vector target and complete `VectorEmbeddingSyncOp` per direct-ingestion row. The Router appends rows after all successful Graph stamps and lifecycle revalidation, before the first typed Vector await; `Progress` removes only the acknowledged prefix, while `Terminal` retains the failed row and later suffix as `Pending`. Pending rows are replayed by the bounded recovery timer, its lost-wake arm latch, and re-armed `post_upgrade`. Router now exposes **54 numbered regions (0–53)**; the outbox is bounded to 1,024 rows with a 2 MiB encoded-row ceiling enforced before insertion, while its value uses `StorableBound::Unbounded` so the transport ceiling does not determine B-tree node pages; the typed Vector driver processes internal chunks of at most 32 rows. |
