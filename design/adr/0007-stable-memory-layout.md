...
Anchor timestamp: 2026-08-24 16:48:04 UTC +0000

## Revision history

| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-25 | Registry truth-telling, no new runtime allocation: `ROUTER_STABLE_LAYOUT` catches up to the Router's already-allocated MemoryIds 54/55. It adds MemoryId 54 `ROUTER_INDEX_RETIRED` (Canonical, `RebuildPath::None`, VersionedSurvivor — the value's bare-Candid encoding has no version envelope today and the gap is deliberately visible in the registry) and MemoryId 55 `ROUTER_AUTH_GRANT_ROWS` (Canonical, `RebuildPath::None`, VersionedSurvivor satisfied by `GrantRow`'s hand-written tag codec). The typed registry now pins **56 numbered regions (0–55)**, matching the memory-manager policy inventory; §2 summary updated. |
| 2026-08-22 | Plan 0278 allocates the vector canister's retired MemoryId 11 hole as `VECTOR_SLAB_COMPACTION_STATE` (Maintenance), the single global record `{ write_cursor, range_end, scan_cursor, pages_moved }` behind the opt-in bounded slab dead-space compaction driver. The vector canister has **18 numbered regions (0–17; 17 allocated)**; only MemoryId 8 remains unallocated, and no neighboring id moved.                                                                                                                                    |
| 2026-08-22 | Router adds `ROUTER_INDEX_RETIRED` (MemoryId 54), one `StableBTreeMap<RetiredPhysicalIndexKey, RetiredIndexRecord>` keyed by the monotonic never-reused physical posting namespace, for the durable `DROP INDEX` retirement obligation (ADR 0023 D6): the value freezes the drain target set resolved before the destructive catalog mutation and persists one resume cursor per pending graph-index target until each confirms `done`, then the record deletes. Router now exposes **55 numbered regions (0–54)**; the memory-manager policy inventory test enforces the count and contiguity. |
| 2026-08-22 | Router MemoryId 53 remains the sole direct-ingestion lifecycle owner and now stores `AwaitingGraph \| AwaitingVector \| AwaitingFrontier`. Graph rejection and observed Vector-prefix resolution retain exact rows as frontier markers; Router derives marked target/shard frontiers from the oldest unresolved exact-lane intent or the durable allocation ceiling, and retires only the unchanged marker snapshot after an observed Vector acknowledgement. The Router-only Vector endpoint applies the frontier monotonically to MemoryId 15 and runs one bounded GC step in the same no-await update. The focused frontier PocketIC and benchmark gates pass, and the unfiltered persisted canbench evidence is terminal. The targeted affected-crate format check passes; the workspace-wide format check is blocked by unrelated dirty paths and never passed. No new stable region is allocated. |
| 2026-08-21 | Router MemoryId 53 is the canonical fresh-install `StableBTreeMap<VectorIngestOutboxKey, VectorIngestOutboxValue>` for direct ingestion. The fixed 43-byte key is ordered by immutable mutation id, exact Vector target, shard, and phase; the compact value carries Graph target/metadata, subject, and embedding bytes. One synchronous admission operation co-writes the mutation counter and exact `AwaitingGraph` keys/values before the first Graph await; phase transitions move the exact key/value pair without an await. The bounded recovery timer and `post_upgrade` replay exact rows. Router exposes **54 numbered regions (0–53)**; the outbox is bounded to 1,024 rows with a 2 MiB encoded-row ceiling enforced before insertion, while its value uses `StorableBound::Unbounded` so the transport ceiling does not determine B-tree node pages. |
| 2026-08-20 | Graph adds `INDEX_PENDING_FLOOR` (MemoryId 52), one fixed-key `StableBTreeMap` that projects the exact oldest tracked ordinary row across `INDEX_REPAIR_JOURNAL` (41) and `DERIVED_INDEX_OUTBOX` (46). Graph now exposes **53 numbered regions (0–52)** with 17 allocated facade regions; region 52 is derived and is synchronously co-updated with its two durable owners. The current install layout has no alternate scan, rebuild, or compatibility path. |
| 2026-08-11 | [ADR 0064](0064-vector-index-architecture.md) §5 adds `VECTOR_DELETED_SUBJECTS` (MemoryId 17) and `VECTOR_GC_CURSOR` (MemoryId 16) as tombstone-retention maintenance bookkeeping. Production deletion remains paused while the Router watermark is zero; the vector canister has **18 numbered regions (0–17; 16 allocated)**. |
| 2026-08-08 | [ADR 0064](0064-vector-index-architecture.md) §5 adds the vector canister's per-shard watermark pair `VECTOR_SHARD_WATERMARKS` (MemoryId 15) and the subject-map tombstone-retention cursor `VECTOR_GC_CURSOR` (MemoryId 16). The current contract supersedes the earlier assumption that Router delivery advances `router_watermark`: only the attached Graph's contiguous applied prefix advances `graph_watermark`, Router watermark remains zero, and tombstone deletion is conservatively paused pending a future contiguous frontier. Vector canister now has **17 regions (0–16)**. |
| 2026-08-08 | [ADR 0064](0064-vector-index-architecture.md) retires the graph's canonical `VERTEX_EMBEDDINGS` (44) and `VERTEX_EMBEDDING_INCARNATIONS` (45) regions: the vector canister owns indexed embedding bytes, while Router direct-ingestion retains pending/retry payload bytes until exact marker retirement; the `mutation_id` ordering fence replaces the incarnation clock. Both become reserved holes (`RESERVED_44`/`RESERVED_45`); graph region count stays **52 (0–51)**. The graph's embedding store, `dispatch_vertex_upsert`, and the embedding backfill are removed; the Router uses the two-call flow (Graph `stamp_embedding` then Vector persist).                                |
| 2026-08-06 | `BulkLoadChunkReceiptRecordV1` stops persisting the chunk envelope and compacts completed rows in place (dropping the resolved Graph request and its fingerprint), so `bulk_load_status` / finalize / receipt-GC decode only receipt-sized rows; Router regions stay **52 (0–51)** — a previously proposed slim receipt projection at MemoryId 52 was reverted in favor of this compaction. The stable-layout benchmark touch set now includes the bulk-load receipt region (49) and the index-catalog epoch cell (51), which were previously omitted.                                                |
| 2026-08-03 | [ADR 0059](0059-create-index-migration-backfill.md) adds Router `ROUTER_INDEX_CATALOG_EPOCH` (MemoryId 51) for the monotonic physical-index lifecycle fence and repurposes Router MemoryId 20 from the removed `ROUTER_INDEXED_PROPERTY_SET` to the `ROUTER_NEXT_PHYSICAL_INDEX_ID` allocator cell; Router now has **52 regions (0–51)**. Graph gains `CANONICAL_EXPORT_SCOPES` (MemoryId 51) for **52 regions (0–51)**; graph-index gains `INDEX_BUILD_STATES` (7) and `INDEX_BUILD_TOUCHED_SUBJECTS` (8) for **9 regions (0–8)**. The stable-layout benchmark touch set must include the new cells. |
...
| 2026-08-02 | [ADR 0058](0058-versioned-additive-schema-migrations.md) adds Router `ROUTER_SCHEMA_MIGRATIONS` (MemoryId 50) for the canonical versioned migration ledger; Router now has **51 regions (0–50)**. No existing region is repacked; the stable-layout benchmark touch set must include the new region.                                                                                                                                                                                                                                                                                                  |
...
| Graph — facade      | 17           | 32–52    | Allocated regions 32–46, 51, and 52; regions 47–50 remain reserved. MemoryId 52 is the exact derived ordinary index-pending floor, synchronously co-updated with owner regions 41 and 46.                                                                                                                                                                                                                                                    |
| Router              | 56           | 0–55     | Grouped auth → registry → runtime config → idempotency → catalog → telemetry → maintenance → constraint catalog/reservations (ADR 0030, 34–39) → embedding/vector catalogs (40–44) → provisioning state (45–48) → ADR 0057 durable bulk-load chunk receipts (49, completed rows compacted in place) → ADR 0058/0059 schema-migration ledger (50) → ADR 0059 index-catalog epoch (51) → ADR 0065 vector-index allocator (52) → direct vector-ingestion outbox (53) → retired-index purge obligations (54) → data-plane grant rows (55); `ROUTER_GRAPH_RUNTIME_CONFIG` remains at MemoryId 5 |
| Graph-index         | 9            | 0–8      | Router auth, shard catalog, ownership config, derived postings, then ADR 0059 physical-index build state (7) and touched-subject set (8)                                                                                                                                                                                                                                                                                                  |
| Vector canister     | 17 allocated / 18 slots | 0–17 | Router auth, shard catalog, ownership config, index defs, centroid meta, reserved centroids, subject clock, partition heads, page meta + row slab (ADR 0064 §7 two-table format; MemoryIds 10/13), rebuild lifecycle state (ADR 0031 Slice 7), maintenance scan state, per-shard watermarks, `VECTOR_GC_CURSOR` (`Option<DeletedSubjectKey>`), `VECTOR_DELETED_SUBJECTS` tombstone-retention list (ADR 0064 §5), and the plan 0278 `VECTOR_SLAB_COMPACTION_STATE` driver record at the retired MemoryId 11; MemoryId 8 unallocated (retired id reverse map). IVF projections rebuild from active Vector rows; full durable-row loss requires client re-ingestion. |
...
| Policy               | Choice                                                                                                                                                                                   |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Pre-production state | Reinstall when the single canonical layout changes                                                                                                                                        |
