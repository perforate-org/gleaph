...
| Graph-vector-index | 13 | 0–12 | Router auth, shard catalog, ownership config, index defs + allocators, centroid meta, reserved centroids, subject clock, id→slot, partition heads, pages (ADR 0031 Slice 2), id→subject reverse locator (ADR 0031 Slice 6), rebuild lifecycle state (ADR 0031 Slice 7) |
...
| Router placement vs label stats projection | *(removed)* Router placement retired ADR 0017; label stats projection remains separate from catalog |
