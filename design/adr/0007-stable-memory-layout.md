...
| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-21 | Router MemoryId 53 is the canonical `StableBTreeMap<u64, VectorIngestOutboxState>` for direct ingestion. One synchronous admission operation co-writes the mutation counter and exact `AwaitingGraph` rows before the first Graph await; each row contains immutable Graph/Vector targets, metadata, subject, bytes, and an `AwaitingGraph \| AwaitingVector` phase. Exact Graph acceptance advances only the phase, rejection resolves only the row, unknown outcomes retain it, and typed Vector acknowledgement removes only the applied prefix. The bounded recovery timer and `post_upgrade` replay exact rows. Router exposes **54 numbered regions (0–53)**; the outbox is bounded to 1,024 rows with a 2 MiB encoded-row ceiling enforced before insertion, while its value uses `StorableBound::Unbounded` so the transport ceiling does not determine B-tree node pages. |
