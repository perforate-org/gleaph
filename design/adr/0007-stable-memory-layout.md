...
| Vector canister     | 16 allocated / 18 slots | 0–17 | Router auth, shard catalog, ownership config, index defs, centroid meta, reserved centroids, subject clock, partition heads, page meta + row slab (ADR 0064 §7 two-table format; MemoryIds 10/13), rebuild lifecycle state (ADR 0031 Slice 7), maintenance scan state, per-shard watermarks, `VECTOR_GC_CURSOR` (`Option<DeletedSubjectKey>`), and `VECTOR_DELETED_SUBJECTS` tombstone-retention list (ADR 0064 §5); MemoryIds 8/11 unallocated (retired id reverse maps). IVF projections rebuild from active Vector rows; full durable-row loss requires client re-ingestion. |
