...
| Graph-vector-index  | 15           | 0–14     | Router auth, shard catalog, ownership config, index defs + allocators, centroid meta, reserved centroids, subject clock, id→slot, partition heads, page meta + row slab (ADR 0064 §7 two-table format; MemoryIds 10/13), id→subject reverse locator (write-maintained only), rebuild lifecycle state (ADR 0031 Slice 7), maintenance scan state (ADR 0031 Slice 10)                                                                       |
