...
| Router              | 52           | 0–51     | Grouped auth → registry → runtime config → idempotency → catalog → telemetry → maintenance → constraint catalog/reservations (ADR 0030, 34–39) → embedding/vector catalogs (40–44) → provisioning state (45–48) → ADR 0057 durable bulk-load chunk receipts (49) → ADR 0058/0059 schema-migration ledger (50) → ADR 0059 index-catalog epoch (51); `ROUTER_GRAPH_RUNTIME_CONFIG` remains at MemoryId 5 |
