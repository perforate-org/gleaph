...
| Spare capacity is a hint (landed 2026-09-20) | `force_slack_grow` only widens a span when the current window can already host the wider span; when the content fits (`min_required <= old_alloc`) and the extra room is not free, the rewrite keeps `old_alloc` and publishes in place. A proactive-slack request therefore never triggers a base resolution, a leaf relocation or a failure, and it must not be fed back into resident sizing (that made resident geometry grow every round). Only content that does not fit may move a leaf. | `compact.rs` (both sizing paths, before base resolution) |
| Spanless vertices request nothing (landed 2026-09-20) | A vertex whose buckets are all tiny owns zero slab slots (§4b/§5), so it contributes no resident term in *either* sizing path (the planner skips tiny explicitly; the rebalance no longer applies its degree floor to them), a `min_required == 0` rebalance short-circuits before resolving a base, and the planner never asks for slack when `total_live` is zero. Without this, a spanless vertex requested a real span, and when it could not be placed the slack tail had no anchor (`labeled_edge_base_from_first_bucket` requires a non-tiny bucket), failing the whole leaf-wide log recovery. | `compact.rs` (both sizing paths, rebalance short-circuit) |
| Span width ownership (landed 2026-09-20) | Invariant I1: the width a caller publishes is the width the base resolution obtained, so `resolve_labeled_edge_base_for_growth` and `resolve_labeled_edge_base_for_rebalance` return `(base, width)`. `for_rebalance`'s fallback (`labeled_edge_base_from_first_bucket`) reports the span the vertex already holds — a value a relocation may itself have grown — instead of the caller's request. Publishing the request over a kept base is the phantom cover that can overlap a leaf mate. | `compact.rs` (`resolve_labeled_edge_base_for_*`, `rewrite_vertex_edge_span_read_and_plan`, `rebalance_vertex_edge_span`) |
| Log-segment release ownership (landed 2026-09-20) | A leaf's overflow-log segment is released only after every bucket in that leaf is drained: `release_leaf_overflow_log` runs `ensure_leaf_overflow_logs_drained` (walking `overflow_log_head` for each bucket of each vertex in the leaf) and refuses with `LaraOperationError::LogSegmentNotDrained` otherwise. The store cannot own this check — `LogStore::release_segment` zeroes the segment's entries and resets its index, and that index is a high-water mark that legitimately stays positive after folds. This is the guard between the never-fail base-resolution fallback (a fold that could not complete because the span could not grow) and silent row loss. | `compact.rs` (`release_leaf_overflow_log`, `ensure_leaf_overflow_logs_drained`), `bucket.rs` (sole-active-vertex release) |
