...
validates the projection against the inline property byte width and decodes the inline property bytes into a
declaration-ordered `Value::Record`. Struct mutation packing, `COST BY` over a struct field,
property indexes on inline struct fields and nested structs remain planned.
