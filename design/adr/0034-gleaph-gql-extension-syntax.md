...
> vector canister routing, inline property byte profiles, and maintenance APIs must not leak into the public
> query language.
...
- `gleaph-graph` owns shard-local graph execution, inline edge inline property decoding, and runtime functions
...
Document vector search in ADR 0031, edge inline property syntax in ADR 0008, IC values in `gql-ic`, and
...
Expose vector search, maintenance, inline property bytes reads, and runtime operations as `CALL GLEAPH.*`.
...
20. Add scalar `INLINE` edge-property schema registration as a Router-owned Graph Type path (ADR 0034 Slice 20, implemented). The `gleaph`-gated edge property form `CREATE GRAPH TYPE <name> { ... EDGE ... { <property> <fixed_scalar_type> INLINE } CONNECTING ... }` is parsed into the edge type AST, applied when the Graph Type is bound to a graph, interned through the existing label/property catalogs, and persisted as a versioned `EdgeInlinePropertySchemaRecord` in `ROUTER_EDGE_INLINE_PROPERTY_PROFILES`. The physical `EdgeInlinePropertyProfile` is derived from the scalar type and travels on the existing `ResolvedEdgeLabel` wire to Graph, so inline property DML/execution consumes the declared width/encoding without any Graph-side schema ownership change. Exactly one inline slot per edge label is enforced; exact replay is idempotent; conflicting declarations return `Conflict`; the `UnnamedProfile` variant remains for admin-installed profiles and cannot be silently overridden by the admin setter. The former standalone `CREATE EDGE LABEL` Router parser is removed.

21. Add ordinary read access to a Slice 20 scalar inline property through standard `e.property` syntax (ADR 0034 Slice 21, implemented). Router projects the scalar inline schema (`ResolvedInlineSchema::Scalar { property_id }`) onto `ResolvedEdgeLabel.inline_schema`; Graph resolves the named property through `ResolvedPropertyTable`, matches it against the concrete edge label's inline slot, and strictly decodes the bound edge inline property bytes into the exact GQL scalar value. For the matching inline property, inline property bytes are the only read source; malformed or missing inline property bytes fail closed and a sidecar property value cannot override or rescue the read. Projection, `WHERE`, comparisons, aggregate inputs, and `ORDER BY` reuse one shared inline-aware read helper. Scalar inline properties and struct leaf fields may have an edge Property Index.

22. Add ordinary mutation packing of scalar values into a Slice 20 inline property bytes through standard GQL edge mutations (ADR 0034 Slice 22, implemented). For a concrete Router-resolved edge label with an `InlineScalar` schema, `INSERT` requires exactly one assignment for the named inline property, evaluates and validates it before creating any adjacency record, encodes it into the fixed-width inline property bytes, and inserts the edge through the existing directed/undirected inline-property-bytes-aware path. `SET e.inline_property = <expr>` and `SET e = { ... }` re-resolve the concrete label, require and encode the inline property exactly once, and update the inline property bytes through the existing mirrored forward/reverse/undirected commit. `REMOVE e.inline_property` is rejected because this slice has no absence representation. Non-inline properties on the same edge retain existing sidecar storage and index-maintenance behavior. When a scalar inline property is indexed, Graph decodes the canonical inline bytes and emits the same old-posting removal/new-posting insertion events used by sidecar properties; edge deletion and slot relocation emit the corresponding removal and re-key events. Graph uses one shared scalar codec for encoding (mutation), decoding (read), and raw predicate-byte preparation; no second schema table or sidecar fallback is introduced. Invalid, missing, duplicate, or `NULL` inline propertys fail closed before any canonical write.
...
`ORDER BY`. Nested fixed-size record paths are flattened in the physical schema and reconstructed at
the GQL value boundary. Router derives a bounded physical field projection from the canonical declaration;
Graph validates the projection against the inline property byte width and decodes the inline property bytes
into a declaration-ordered `Value::Record`. Full and field-level struct mutation and `COST BY` over a
struct field are implemented. Inline struct leaf indexes accept the same dotted field path in
`CREATE INDEX ... ON (e.stats.score)`; Router interns that path as the posting property identity,
projects the path and edge direction in the per-operation catalog, and Graph decodes the matching
leaf from canonical inline property bytes for insert, update, delete, and slot relocation.
