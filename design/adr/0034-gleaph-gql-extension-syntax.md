...
12. Add exactly one equality predicate and two same-property numeric range predicates (one lower and one upper) on a distinct property of the searched vertex for leading and non-leading `SEARCH ... WHERE` (ADR 0034 Slice 12, done). The planner accepts the three-leaf mixed shape with any conjunct order and either operand order; the Router independently revalidates, resolves both the equality property id and the single range property id, proves active vertex property indexes for the same label, encodes the equality value once, collapses the two range arms into one intersected finite half-open encoded interval through the existing `resolve_filtered_range_interval` path, and collects label-qualified candidates through the same bounded `lookup_range_intersection_page` stream used by Slice 11. An empty or contradictory interval short-circuits before any Property Index or Vector Index call while preserving the leading/non-leading aggregate dispatch contract. No new Property Index API, transport type, persisted state, or Vector Index behavior is introduced.
