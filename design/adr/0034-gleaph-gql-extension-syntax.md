...
Date: 2026-06-25
Status: accepted (syntax design; Rust manifest implemented; SEARCH parser/planner and Router lowering implemented, including Slice 6 leading labeled `SEARCH ... WHERE` equality filter, Slice 7 non-leading `SEARCH ... WHERE` equality filter, and Slice 8 one or two `AND`-connected same-binding equality conjuncts for both leading and non-leading search; remaining syntax staged by feature)
