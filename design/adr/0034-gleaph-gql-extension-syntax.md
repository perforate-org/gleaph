...
17. Add bounded same-property numeric range disjunctions (2..=8 OR-connected arms) for leading and non-leading `SEARCH ... WHERE` (ADR 0034 Slice 17, done). The Planner accepts any number of `OR`-connected range predicates on the searched binding as long as every arm is a pure numeric range comparison (`<`, `<=`, `>`, `>=`) between the same property of that binding and a literal or parameter, with no equality, no different property, and no nested logical operator. The provider-neutral contract is "arbitrary-length pure same-property numeric range disjunction". The Router classifies this shape as `RangeDisjunction`, enforces a separate Router-owned execution bound of eight arms, requires one active vertex property index for the shared `(graph_id, label_id, property_id)` tuple, and resolves each arm to a finite half-open encoded interval through `gleaph_gql::numeric_range_bounds`. Empty or contradictory arms are dropped; the remaining intervals are sorted and merged into a minimal set of disjoint encoded intervals. The candidate set is built by walking every merged interval on every index source through paginated `lookup_range_page` with `PostingRangeRequest::Between { low, high }`, label-filtering each page before counting, and merging results with global `(shard_id, vertex_id)` deduplication. The same 4096 candidate bound is enforced fail-closed. A single range predicate remains the existing `Range` path; mixed OR/AND, OR across different properties, equality inside OR, and nine or more arms remain fail-closed. Empty candidate sets preserve the existing leading/non-leading aggregate dispatch contract. The union logic lives entirely in the Router and reuses the same shared collector abstraction as the equality disjunction path; neither the Property Index endpoint contract nor the Vector Index behavior changes.
