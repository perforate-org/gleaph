...
to about 56 MiB, but leaves a 16 GiB ceiling for that manager. The current global
16-page setting raises that ceiling to 32 GiB, while still applying a 1 MiB minimum
allocation to every growing region. Preliminary property-heavy workloads also show
...
- The manager metadata will receive a new layout version. The old global-bucket
  metadata will not be reinterpreted by the new implementation. Development data
  may be recreated; production rollout requires the migration, capability, and
  preflight procedure in ADR 0039.
...
1. Add the custom crate with mixed-size metadata, extent allocation, reopen, grow,
   failed-grow, and cross-boundary tests. Port the upstream manager tests that
   cover independent growth, cache behavior, maximum allocation, and reopen.
...
Until these steps complete, the 16-page global Graph manager is the implemented
interim layout, and this ADR describes planned architecture rather than a shipped
