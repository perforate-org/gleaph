...
| `FWD_INLINE_PROPERTY_BYTES_SLAB`, `REV_INLINE_PROPERTY_BYTES_SLAB`, `FWD_INLINE_PROPERTY_BYTES_LOG`, `REV_INLINE_PROPERTY_BYTES_LOG`, `FWD_INLINE_PROPERTY_BYTES_BLOBS`, `REV_INLINE_PROPERTY_BYTES_BLOBS` | 32 | retain | Inline and overflow values are larger and grow with edge volume; inline property bytes domains should not compete for the 4-page extent budget. |
| `FWD_INLINE_PROPERTY_BYTES_FREE_SPANS`, `FWD_INLINE_PROPERTY_BYTES_FREE_SPAN_BY_START`, `REV_INLINE_PROPERTY_BYTES_FREE_SPANS`, `REV_INLINE_PROPERTY_BYTES_FREE_SPAN_BY_START` | 4 | retain | Rebuildable free-span indexes are sparse maintenance state. |
| `MAINTENANCE_QUEUE` | 4 | retain | Bounded/deferred maintenance state; large pages would only hide queue pressure as slack. `DIRTY_WORK_ITEMS` was retired in ADR 0052 Slice 6 (the priority-ordered `StableBTreeMap` queue replaced the dirty bitmap). |
