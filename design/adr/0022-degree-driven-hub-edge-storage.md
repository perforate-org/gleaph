...
2. **Stage 2 (deferred): degree-driven hub edge storage (Alternative D).**
   ...
   **Both tiers' necessity and degree thresholds are decided by benchmarks** — the
   ...
   - **Target lives in the value, not the key.** A dedicated target struct holding
     only `{ remote flag, id }` (tombstone/inline property bytes bits stripped from `VertexRef`,
   ...
   - **Value reuses the existing edge-inline-property representation / `EdgeInlinePropertyBytesStore`
     indirection** (single source of truth for inline property schema), not a new
...
1. Benchmark shared-leaf vs dedicated-span vs B-tree backing for skewed buckets:
   ...
   - **Value-layout experiment landed** (`bench_labeled_stage2b_narrow_*` and
     ...
     already externalized to `EdgeInlinePropertyBytesStore`/`EdgePropertyStore`), so the value is
   ...
   **Value-layout experiment verdict (2026-06-19):** the read regression is **not** a
   ...
   4 bytes (the real `Edge` width) or splitting target and inline property bytes into separate trees
   gives no scan benefit, because production edge inline properties are already external (the
   ...
   two-tree / inline-property-bytes-split idea for the hub tier, and do not expect a packed value to
  ...
  **Dual case measured — reverse mirror index not warranted (2026-06-20).** The dual of
  ...
  recorded if that regime ever bites: (a) skip `attach_edge_inline_property` in
