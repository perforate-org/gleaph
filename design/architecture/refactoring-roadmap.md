...
**Progress (2026-06-10):** `graph-index` and `router` facades split into storage-domain submodules under `facade/store.rs`. Graph `GraphStore` adds `adjacency` (insert and delete commit), `properties`, and `labels` domain methods; read APIs remain in domain-specific modules.
