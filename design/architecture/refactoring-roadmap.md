...
**Progress (2026-06-10):** `graph-index` and `router` facades split into storage-domain submodules under `facade/store.rs`. Graph `GraphStore` adds domain commits for adjacency, properties, labels, vertex delete, remote refs, telemetry, edge profiles, and local indexes; router adds `backfill` domain. Read APIs remain in domain-specific modules.
