...
**Remaining (non-blocking):** native standalone dev may still wire `PropertyIndexLookup` for local tests; remove legacy `execute_index_intersection` N× `lookup_equal` fallback once all dispatch uses router seeds; full router `gql_query` → index → graph PocketIC path.
