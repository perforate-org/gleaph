...
| ------------------ | ------- | -------- | ------------------------------------------------------------------------------ |
| Graph              | 55      | 0–54     | `GRAPH_STABLE_LAYOUT` — `graph_layout_registry_matches_baseline`               |
...
| Provision          | 16      | 0–15     | `PROVISION_STABLE_LAYOUT` — `provision_layout_registry_matches_baseline`       |
...
**Plan 0338 LTB region + policy reconciliation (2026-09-07).** The ADR 0088 LTB regions are now
recorded: graph MemoryId 53 (`FWD_LTB`) and 54 (`REV_LTB`), raising the graph registry to
**55 numbered regions, 0–54** (verified against `GRAPH_STABLE_LAYOUT`, which lists `fwd_ltb` at
53 and `rev_ltb` at 54). Policy discrepancy resolution: ADR 0088 §1's "64-page VMM bucket
policy" statement is **superseded by the code** — the wired `GRAPH_MEMORY_MANAGER_POLICIES`
carries `(FWD_LTB, 16)` / `(REV_LTB, 16)`. As the implementer of the manager semantics
(`ic-stable-variable-memory-manager`), the policy value is the per-memory **extent allocation
granularity in pages, not a capacity cap** (`grow()` rounds the memory size up in whole extents
and appends them to a global extent list; the global `MAX_EXTENTS = 65,536` budget, not the
policy value, bounds capacity). A 16-page extent granularity for the LTB keeps extent-owner
metadata proportionate to the block-stride (4,112 B) write pattern while avoiding the
64-page-granularity over-allocation that pure block stores would suffer on sparse trees; the
ADR text is the stale side and is corrected by ADR 0094's implementation-status record rather
than a code change. Stable-memory compatibility is not maintained for this development layout
(existing dev data recreated — ADR 0039 fresh-state policy).
...
| 49 | `ROUTER_BULK_LOAD_CHUNK_RECEIPTS` | `ROUTER_BULK_LOAD_CHUNK_RECEIPTS` | `init_bulk_load_chunk_receipts` | canonical | durable bulk-load receipts | **`(bulk_job_mutation_id, chunk_index) → BulkLoadChunkReceiptRecordV1`** ([ADR 0057](../adr/0057-router-operation-api-and-durable-bulk-load.md)). Owns the chunk **fingerprint** (`[u8; 32]`) as the resume idempotency key, plus the pinned Graph request and child mutation ID, exact canonical/public receipt, progress, and completion anchor. The chunk envelope itself is **not** persisted — its only role was read-time re-derivation of the digest, which the Graph-request fingerprint handshake already covers for replay correctness. Update chunks (`BulkLoadChunkV1::Updates`) carry no Graph request; their row carries `updated_row_count` as the durable committed prefix (retained on a completed row as its receipt count, monotonic and bounded by the admitted row count at its write boundary) and `resolved_update_vertex_ids` as the fixed execution targets and admitted row count (compacted away on completion). The per-row `client_mutation_key` of an update row is the existing Router saga record (plus the Graph mutation journal for that mutation id), so row-level resume, dispatch-permission, and abort classification need no bulk-load-owned journal field; the parent `AppendPending`/`AbortPending` lifecycle is the durable dispatch gate. Terminal parent retention is anchored in region 7; bounded receipt GC deletes at most 32 consecutive rows per step, persists its cursor in the parent, and removes the parent binding only after an empty-range proof. |
...
there is no compatibility decoder and no additional region.
...
then to 59 bytes (schema `GLEAPH-VECDEF-03`, adding `code_tier`/`code_stride_bytes`/`rotation_seed`,
Slice 6 / ADR 0079), then to **67 bytes (schema `GLEAPH-VECDEF-04`, adding the frozen per-level ε₂
`eps_query_bps` / `eps_fine_bps` pair, Slice 9)**; fresh install required for all schema changes, with
...
| 18       | `VECTOR_REBUILD_POOL`                  | — (raw region)             | — (`rebuild_pool::begin`)     | derived     | single-tenant raw rebuild-pool region (**ADR 0033 implementation**): frozen Sampling/Training candidate rows (pad-stride stored bytes + aux), the Training centroid work area, and — two-level rebuilds only (Slice 5) — the per-row coarse-id array behind a minimal `VRP`/**version-4** header (binding + `pool_len`/`pool_capacity`/`centroid_count`/`assigned_len` + Slice 6 `code_tier` flag byte + Slice 9 frozen `eps_query_bps`/`eps_fine_bps` pair carrying the shadow generation's tier and per-level ε₂ pruning from start to `Building`/`publish`, **ADR 0079**); fail-closed resume validation against the lifecycle scalars; released at abort entry, teardown completion, `Failed`, and coordinated reset. Fresh layout cutover (format version 4, breaking over earlier headers): no migration or compatibility reader | `admin_start_vector_rebuild`                                                               |
