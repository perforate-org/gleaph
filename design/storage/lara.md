...
- [adr/0049-input-order-preserving-batch-graph-mutations.md](../adr/0049-input-order-preserving-batch-graph-mutations.md) — **partially implemented**; retains the ordered placement/reservation/commit substrate for labels whose Graph Type policy is `ORDER BY INSERTION`. The per-label default-unordered policy, tombstone-first batch placement, and policy-specific swap-compaction are defined by [ADR 0052](../adr/0052-per-label-adjacency-order-and-tombstone-reuse.md). LARA does not own the Graph Type policy or the cross-canister mutation contract.
- [adr/0052-per-label-adjacency-order-and-tombstone-reuse.md](../adr/0052-per-label-adjacency-order-and-tombstone-reuse.md) — planned per-label ordering policy, unordered tombstone reuse, hole-first batch placement, and unordered swap-compaction.
- [adr/0088-tree-csr-mode-for-high-degree-label-buckets.md](../adr/0088-tree-csr-mode-for-high-degree-label-buckets.md) — **accepted (2026-08-29)** tree-CSR mode for high-degree label buckets (LTB block store, root array in the CSR span, derived depth/geometry, bounded per-bucket leaf footprint); supersedes ADR 0022 Stage 2. Implementation is gated on its measurement plan; nothing in the current layout changes until implementation lands.
- [adr/0094-ltb-block-tombstone-count-level1-offset.md](../adr/0094-ltb-block-tombstone-count-level1-offset.md) — **accepted (2026-09-07)** per-block u16 tombstone count in the LTB block header (offset 13-14; stride/version unchanged, no migration under the not-deployed fresh-state policy); single-funnel maintenance with fail-closed scan self-verification; production tree window walk to be restructured as window-restricted block resolution + in-window dead-block skip. Slab/bypass regimes stay status quo; Level 2 directory and tree tombstone reuse are deferred.
