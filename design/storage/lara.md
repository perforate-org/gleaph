...
- [adr/0049-input-order-preserving-batch-graph-mutations.md](../adr/0049-input-order-preserving-batch-graph-mutations.md) — **partially implemented**; retains ADR 0045's implemented placement/reservation/commit substrate and the active input-order-preserving edge API. The Router mixed public shape, immutable replay target/identity persistence, Graph-owned immutable mixed envelope, Graph journal identity/aggregate receipt wire shapes, pure phase planner, Graph handler execution, Router catalog resolution, Graph dispatch, and canonical receipt transition are implemented, including request-local new-vertex ordinals and a separate mixed fingerprint domain. Projection, retirement, and recovery remain planned; LARA does not own the mixed request or its two-phase failure contract.
