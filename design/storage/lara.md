...
- [adr/0048-adaptive-lara-mate-index.md](../adr/0048-adaptive-lara-mate-index.md) — accepted physical-pairing design; Plan 0132 adds one-pass live-slot traversal and exact scalar location consumption for supported named buckets. Alias removal is primarily a persistent-bytes optimization: the raw alias payload is 18 bytes per entry (excluding B-tree/allocator overhead), while MemoryManager page deltas are not per-edge measurements and ScanOnly instruction cost is only a guardrail
