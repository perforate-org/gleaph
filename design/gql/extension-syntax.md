...
| SQL-compat string predicates  | `x LIKE pattern` / `x ILIKE pattern` (`[NOT]` forms)                      | **Implemented:** SQL wildcard semantics over Unicode scalars (`%` any run, `_` one scalar, `\` escapes the next pattern scalar, default escape backslash, no `ESCAPE` clause). `LIKE` is case-sensitive; `ILIKE` folds both operands with Unicode lowercase first. Neither keyword exists in GQL core (which has `STARTS WITH`/`ENDS WITH`/`CONTAINS` only) — this is a SQL-compat dialect extension behind the `cypher` feature. Fusion: non-negated vertex `x LIKE <Text literal>` with a non-empty escape-resolved literal prefix fuses into the existing `TextPrefix` anchor (full pattern stays residual for rechecking); `ILIKE`, `$param` LIKE, NOT LIKE, and leading-wildcard/empty-prefix LIKE stay residual-only (`STARTS WITH` keeps its own prefix-index path); non-negated edge `e.prop LIKE <Text literal>` with a non-empty escape-resolved prefix fuses into `EdgeIndexScan { TextPrefix }` symmetrically with the vertex path (full pattern stays residual; ambiguous edge labels and unindexed properties stay residual-only; vertex/edge coexistence is exclusive to the leading-edge slot). GQL text has no `\%` string escape, so a LIKE escape is spelled `\\` in query text (`LIKE 'a\\%b%'` matches literal `a%b…`). Migration note: `ILIKE` previously matched whole-string equality with literal `%`/`_`; those patterns are wildcards now, use `\%`/`\_` (spelled `\\%`/`\\_` in GQL text) for literals | Parser + Graph string-predicate executor |
