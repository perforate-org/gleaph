...
predicates, two to eight `OR`-connected same-property equality predicates, two to eight `OR`-connected pure equality
predicates on **distinct** or repeated properties, or two to eight `OR`-connected same-binding numeric range
predicates on the same or different properties, on both leading and non-leading `SEARCH`. The accepted leading shapes are:
...
`OR`/`XOR`/`NOT` other than the supported same-property or cross-property pure equality `OR` or the supported same-property or cross-property range `OR`, nine or more equality conjuncts, mixed equality/range forms outside the accepted shapes, repeated equality properties, duplicate-direction range arms, `NULL`, functions, computed
...
The same one-equality, two-equality `AND`, N-equality `AND` (2..=8 arms), single numeric-range, same-property two-sided-range, mixed equality-plus-range, same-property equality-`OR`, cross-property pure equality-`OR`, same-property numeric-range-`OR`, and cross-property numeric-range-`OR` shapes apply to non-leading
search.
...
- no other `WHERE` in-index filtering (compound `OR`/`XOR`/`NOT` other than the supported
  same-property or cross-property pure equality `OR` or the supported same-property or cross-property range `OR`,
  two numeric ranges on different properties in a conjunction, text/bytes/temporal/boolean/collection/path/extension-value
