...
| Wasm heap (wasm64 / EOP) | 6 GiB | Query paths must not materialize full buckets in heap (**Implemented** for label, property, and edge equality/range exports via paginated `*_page` reads; equality **intersection** still materializes per-arm sets — see [lookup-intersection.md](lookup-intersection.md#streaming-intersection-status)) |
