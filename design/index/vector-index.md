...
Anchor timestamp: 2026-09-08 13:39:34 UTC +0000
...
| —     | two-tier precision code tier (RaBitQ first stage + same-page exact rerank)         | **implemented (Slice 6, 2026-08-24; ADR 0079)**: flat and two-level both accept it; policy-driven rebuilds default tier on at Router snapshot time (`None` → `Some(true)`, `Some(false)` opts out); the manual rebuild path keeps `None` = off |
