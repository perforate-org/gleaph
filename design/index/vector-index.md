...
Anchor timestamp: 2026-06-29 23:13:38 UTC +0000
...
## Filtered exact ranking (ADR 0034 Slices 6, 7, 8, 9 and 10)
...
equalities on distinct properties, one same-binding numeric range predicate, or exactly two
same-binding numeric range predicates on the same property (one lower `>`/`>=` and one upper
`<`/`<=`) forming a two-sided range) and arrives in `VectorSearchRequest.candidate_subjects`.
