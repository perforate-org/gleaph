...
| `gql_execute_idempotent_batch` | update | bounded page of independent DML mutations | Executes at most 16 `gql_execute_idempotent` items sequentially, preserving one client mutation key and one idempotency boundary per item. Earlier items may be committed when a later item fails; replaying the page is safe through the existing mutation journal. The endpoint is a transport-level bulk primitive, not one multi-DML GQL bundle, so the federated multi-DML gate and shard-local atomicity rules remain unchanged for each item. |
