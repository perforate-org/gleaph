...
5. **Canonical success is not a freshness barrier.** A graph mutation may be durable while a
   ...
   `index_pending_min_mutation_id`. The read-side `AtLeast(token)` barrier that _enforces_ these
...
> **Planned revision (ADR 0056).** [ADR 0056](../adr/0056-router-api-surface-layering-and-consolidation.md) consolidates the
> entrypoints in this table once implemented: `gql_query` + `gql_query_with_consistency` merge into one
> `gql_query(.., read_mode)`; `prepared_query` + `prepared_query_with_consistency` merge into
> `execute_prepared(.., read_mode)`; `force_gql_execute` and `prepared_query_as_update` are removed;
> the idempotent entries are renamed (`gql_execute_idempotent` → `gql_execute`,
> `prepared_update_idempotent` → `execute_prepared_update`);
> `ReadMode::Canonical` is dropped. Until implemented, the table below remains the active contract.
...
`gql_query` / `prepared_query` entrypoints remain `Eventual` (no per-read watermark);
...
| `prepared_query`                  | query (composite) | read-only                                   | Same as `gql_query`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `prepared_query_with_consistency` | query (composite) | read-only                                   | Same as `gql_query_with_consistency`, for a registered prepared plan.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `prepared_query_as_update`        | update            | read-only (escape hatch)                    | Same as `force_gql_execute`, for a registered prepared plan.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `prepared_update`                 | update            | non-DML; DML rejected                       | Same as `gql_execute`, for a registered prepared plan.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `gql_execute_idempotent_batch`    | update            | cursor-based independent DML mutations      | Executes as many remaining `gql_execute_idempotent` items as fit the Router instruction and encoded-payload budgets, unless the optional `max_items` page cap is supplied. Consecutive homogeneous items (same GQL plan and mode) are coalesced into one bulk group sharing a single `MutationId` and one idempotency boundary (ADR 0044) when typed V1 per-operation seeds apply or a seed-invariant shared V2 request fits one Graph request. V2 encodes the common plan, catalogs, seed/search relation, and target once and carries only params per operation. Router bisects an oversized shared V2 group into independently keyed bulk groups until each complete operation domain fits, and returns a continuation after each bisected subgroup; heterogeneous or unsupported items retain a per-item boundary. Earlier items may be committed when a later item fails; replaying from `next_index` is safe through the existing mutation journal. The endpoint is a transport-level bulk primitive, not one multi-DML GQL bundle, so the federated multi-DML gate and shard-local atomicity rules remain unchanged for each item.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `prepared_update_idempotent`      | update            | DML (idempotent)                            | Same as `gql_execute_idempotent`, for a registered prepared plan. The federated multi-DML gate (ADR 0029 §6) runs at `prepared_upsert`, so a federated multi-DML plan can never be stored or reach this entrypoint. The contract-1 anchored single-shard subset is **ad-hoc only**: the leading anchor's runtime shard count is unknown at registration, so anchored multi-DML on a federated graph stays rejected at `prepared_upsert`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
